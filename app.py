...
def init():
    ...
    for s in ss: qexec(s)
    # اندكسات للسرعة
    qexec("CREATE INDEX IF NOT EXISTS idx_subs_id ON subs(id)")
    qexec("CREATE INDEX IF NOT EXISTS idx_dish_id ON dish_ips(id)")
    qexec("CREATE INDEX IF NOT EXISTS idx_tower_id ON towers(id)")
...
def api_ping():
    ...
    try:
        ...
        cmd=['ping','-n','1',ip] if w else ['ping','-c','1','-W','1',ip] # اسرع
        o=subprocess.run(cmd,capture_output=True,text=True,timeout=3)
...
def page_content(v):
    ...
    if v=='dishes':
        rs=qall("SELECT * FROM dish_ips ORDER BY id DESC LIMIT 50") # اسرع
    ...
    if v=='towers':
        rs=qall("SELECT * FROM towers ORDER BY id DESC LIMIT 50") # اسرع
        ...
        for r in rs:
            ...
            h+=f"<div class=card><b>{esc(r['name'])}</b> {fixed}<br><small>{esc(r.get('location',''))}</small><br>احداثيات: {r.get('lat',0)},{r.get('lng',0)}<br><button class=btn>تعديل</button> {delbtn}</div>"
        return h

    if v=='map':
        h="<div class=card><h3>قياس المسافة بين نقطتين</h3><div class=grid-2><input id=lat1 placeholder='خط العرض 1'><input id=lng1 placeholder='خط الطول 1'><input id=lat2 placeholder='خط العرض 2'><input id=lng2 placeholder='خط الطول 2'></div><button class=btn onclick='calcDist()'>احسب المسافة</button><p id=distResult style='font-weight:bold;color:#D4AF37;font-size:18px;margin-top:10px'></p></div>"
...
def layout(c,v='home'):
    ...
    resp = f"""<html dir=rtl lang=ar><head><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'><meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate"><meta http-equiv="Pragma" content="no-cache"><meta http-equiv="Expires" content="0"><title>لوحة تحكم OMAIA</title>
    <style>
  ...
  .avatar{{width:40px;height:40px;border-radius:50%;background:#D4AF37;display:flex;align-items:center;justify-content:center;font-weight:bold}}
    ...
    <div class=top>
      ...
      <div><button class=btn onclick="fetch('/toggle_lang?v=1').then(()=>location.reload())">EN/AR</button></div>
    </div>
    <div class=main id=main>{c}</div>
    <script>
    function toggleMenu(){{document.getElementById('sidebar').classList.toggle('active');document.getElementById('overlay').classList.toggle('active')}}
    window.onload=()=>{{document.getElementById('loader').style.display='none'}}
    window.loadPage=async function(v){{toggleMenu();document.getElementById('loader').style.display='flex';var r=await fetch('/api/page?v='+v+'&t='+Date.now());document.getElementById('main').innerHTML=await r.text();document.getElementById('loader').style.display='none';bindAjax()}}
    ...
    function calcDist(){{let d1=parseFloat(lat1.value),d2=parseFloat(lat2.value),d3=parseFloat(lng1.value),d4=parseFloat(lng2.value);if(isNaN(d1)||isNaN(d2)||isNaN(d3)||isNaN(d4)){{distResult.innerText="ادخل الاحداثيات كاملة";return}}let R=6371,dLat=(d2-d1)*Math.PI/180,dLon=(d4-d3)*Math.PI/180,a=Math.sin(dLat/2)*Math.sin(dLat/2)+Math.cos(d1*Math.PI/180)*Math.cos(d2*Math.PI/180)*Math.sin(dLon/2)*Math.sin(dLon/2),c=2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a)),dist=R*c;distResult.innerText="المسافة: "+dist.toFixed(2)+" كم"}}
    function pingDish(ip){{if(!ip){{alert('لا يوجد IP'));return}}fetch('/api/ping?ip='+ip+'&v=1').then(r=>r.json()).then(j=>alert(j.out.slice(0,300)))}}
    bindAjax();
    </script></body></html>"""
