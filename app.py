...
def notif_count():
    try:
        con=db();c=len(ex(con,"SELECT id FROM notifications ORDER BY id DESC LIMIT 5").fetchall());close_con(con);return c
    except:return 0
...
def render(c):
    ...
    h=h.replace("__NOTIF__",str(notif_count()))
...
def login():
    ...
    if request.method=='POST':
        ...
        if not u:
            if is_phone:
                msg="<p style='color:#f87171'>خطأ في رقم الهاتف او غير موجود</p>"
            else:
                msg="<p style='color:#f87171'>خطأ في اسم المستخدم او غير موجود</p>"
        else:
            d=dict(u) if not isinstance(u,dict) else u
            if not d.get('active'):
                msg="<p style='color:#f87171'>الحساب معطل</p>"
            elif d.get('password')!=pwd:
                if is_phone:
                    msg="<p style='color:#f87171'>خطأ في رقم الهاتف او كلمة السر</p>"
                else:
                    msg="<p style='color:#f87171'>خطأ في اسم المستخدم او كلمة السر</p>"
            else:
...
def search():
    ...
    return render(f"<form method='get'><input name='q' value='{q}' placeholder='بحث عن مشترك...'><button>بحث 🔍</button></form><h3>مشتركين</h3><table>{tr1}</table><h3>صحون</h3><table>{tr2}</table>")
