...
app=Flask(__name__)
app.secret_key=os.environ.get("SECRET_KEY","omia-final-clean-2026")
...
def init_db():
    ...
    for s in ss:
        qexec(s)
    for alter in ["ALTER TABLE dish_ips ADD COLUMN tower_id INTEGER","ALTER TABLE dish_ips ADD COLUMN IF NOT EXISTS tower_id INTEGER","ALTER TABLE towers ADD COLUMN created_at TEXT","ALTER TABLE towers ADD COLUMN IF NOT EXISTS created_at TEXT"]:
        try:
            qexec(alter)
        except:
            pass
    for idx in ["CREATE INDEX IF NOT EXISTS idx_dish_ip ON dish_ips(ip)","CREATE INDEX IF NOT EXISTS idx_dish_tower ON dish_ips(tower_id)","CREATE INDEX IF NOT EXISTS idx_logs_id ON logs(id DESC)"]:
        try:
            qexec(idx)
        except:
            pass
...
def api_ping():
    ...
    for port in [80,443,8080,8291,22,8728,8000,23]:
        ...
        try:
            ...
            if s.connect_ex((ip,port))==0:
                s.close()
                return jsonify(ok=True,out='متصل '+ip+':'+str(port)+' مفتوح')
            s.close()
        except:
            try:
                if s:
    ...
    try:
        ...
        if ok:
            ...
            return jsonify(ok=True,out=ip+' '+ms+'ms')
    except:
        pass
...
def add_dish_to_tower():
    ...
    if ok:
        add_log(session.get('phone'),'إضافة صحن',f"{name} {ip} tower:{tid}")
        with _cache_lock:
            _cache.pop('counts',None)
...
@app.route('/login')
def login():
    return """<html dir=rtl><head><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'><title>OMAIA</title>
...
.card{background:linear-gradient(180deg,#222b45cc,#1a2035cc);border:1px solid #ffffff18;padding:26px;border-radius:22px;width:92%;max-width:380px;animation:fadeUp .8s cubic-bezier(.16,1,.3,1)}
@keyframes fadeUp{from{opacity:0;transform:translateY(24px) scale(.96)}to{opacity:1;transform:translateY(0) scale(1)}}
input{width:100%;padding:14px;margin:9px 0;background:#0f1424;border:1px solid #ffffff22;color:#fff;border-radius:14px;transition:all .5s cubic-bezier(.16,1,.3,1)}
input:focus{border-color:#ffbe4d88;box-shadow:0 0 0 4px #ffbe4d22;transform:scale(1.01);outline:none}
.btn{width:100%;padding:14px;border:0;border-radius:14px;background:linear-gradient(90deg,#ffbe4d,#ffb020);color:#111;font-weight:900;font-size:17px;cursor:pointer;transition:transform .6s cubic-bezier(.16,1,.3,1)}
.btn:active{transform:scale(.92)}
</style></head><body>
<div style='font-size:32px;font-weight:900;margin-bottom:14px'>OMAIA <span style='color:#ffbe4d'>ISP</span></div>
...
def page_content(v):
    ...
    if v=='home':
        ...
        if not log_html:
            log_html='<div style="padding:12px;color:#888">فاضي</div>'
        return f'<div style="max-width:900px;margin:0 auto"><div class=grid2><div class="card stat" onclick="loadPage(\'subs\')"><h3>{L("المشتركين","Subs")}</h3><h2>{ns}</h2><div class=ico>👥</div></div><div class="card stat" onclick="loadPage(\'dishes\')"><h3>{L("الصحون","Dishes")}</h3><h2>{nd}</h2><div class=ico>📡</div></div><div class="card stat" onclick="loadPage(\'towers\')"><h3>{L("الأبراج","Towers")}</h3><h2>{nt}</h2><div class=ico>🗼</div></div><div class="card stat" onclick="loadPage(\'ledger\')"><h3>{L("الحسابات","Accounts")}</h3><h2>{nl}</h2><div class=ico>📒</div></div></div><div class=card style="margin-top:12px"><div class=row style="justify-content:space-between"><b>آخر النشاطات</b><div class=row><button class=btn-gold onclick="fetch(\'/api/seed_log\',{method:\'POST\'}).then(()=>loadPage(\'home\',true))">اختبار</button><button class=btn-gold onclick="loadPage(\'logs\')">الكل</button></div></div><div style="margin-top:8px">'+log_html+'</div></div></div>'
    if v=='towers':
        ...
        return f'<div style="max-width:900px;margin:0 auto"><div class=card><div class=row style="justify-content:space-between"><h3>الأبراج - كروت لا نهائية - {len(rs)}</h3><div class=row><button class=btn-gold onclick="openNewTower()" style="background:#22c55e;color:#fff">+ كرت جديد</button><a href="/api/export/towers" class=btn-gold>Excel</a></div></div></div><div class=grid2>{cards}</div></div><script>window.openNewTower=function(){{let body=document.getElementById("editBody"); body.innerHTML=\'<input id=nt_name placeholder="اسم الكرت" value="كرت جديد"><input id=nt_area placeholder="منطقة"><div class=row><input id=nt_lat placeholder="lat" value="35.1318"><input id=nt_lng placeholder="lng" value="36.7578"></div><button class=btn-gold onclick="saveNewTower()" style="width:100%;margin-top:8px">حفظ</button>\'; document.getElementById("editModal").classList.add("show");}};window.saveNewTower=function(){{fetch("/add_tower",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{name:document.getElementById("nt_name").value,area:document.getElementById("nt_area").value,lat:document.getElementById("nt_lat").value,lng:document.getElementById("nt_lng").value}})}}).then(r=>r.json()).then(j=>{{if(j.ok){{closeEditModal(); loadPage("towers",true)}}}})}};window.openEditTower=function(id){{let c=document.getElementById("tower-"+id); let body=document.getElementById("editBody"); body.innerHTML=\'<input id=et_name value="\'+c.dataset.name+\'"><input id=et_area value="\'+c.dataset.area+\'"><div class=row><input id=et_lat value="\'+c.dataset.lat+\'"><input id=et_lng value="\'+c.dataset.lng+\'"></div><button class=btn-gold onclick="saveTower(\'+id+\')" style="width:100%">حفظ</button>\'; document.getElementById("editModal").classList.add("show");}};window.saveTower=function(id){{fetch("/edit_tower/"+id,{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{name:document.getElementById("et_name").value,area:document.getElementById("et_area").value,lat:document.getElementById("et_lat").value,lng:document.getElementById("et_lng").value}})}}).then(()=>{{closeEditModal(); loadPage("towers",true)}})}};window.addDishToTower=function(tid){{let ip=document.getElementById("ip-"+tid).value.trim(); let nm=document.getElementById("name-"+tid).value.trim(); if(!ip) return alert("IP"); fetch("/api/add_dish_to_tower",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{ip:ip,dish_name:nm,tower_id:tid}})}}).then(r=>r.json()).then(j=>{{if(j.ok) loadPage("towers",true); else alert(j.msg||"خطأ")}})}};window.delDishInTower=function(did,tid){{fetch("/del_dish/"+did).then(r=>r.json()).then(j=>{{if(j.ok) loadPage("towers",true)}})}};window.focusMap=function(lat,lng){{loadPage("map"); setTimeout(()=>{{if(window._map) window._map.flyTo([lat,lng],18)}},600)}};</script>'
    if v=='dishes':
        ...
        for r in rs:
            ...
            tid=r.get('tower_id')
            badge=" • برج "+str(tid) if tid else ""
            rows+=f'<div class="card dish-card" id="dish-{rid}" data-name="{dn}" data-ip="{ip}" data-loc="{loc}"><div><b>{dn}</b><br><span class=ip>{ip}</span><br><small>{loc}{badge}</small></div><div class=col><button class=btn-gold onclick="quickPingD({rid})">Ping</button><div class=row><button class=btn-gold onclick="editDish({rid})">✏️</button><button class=btn-del onclick="askDel(\'/del_dish/{rid}\',{rid})">🗑</button></div></div></div>'
        return f'<div style="max-width:900px;margin:0 auto"><div class=card><div class=row style="justify-content:space-between"><h3>الصحون - {len(rs)}</h3><a href="/api/export/dishes" class=btn-gold>Excel</a></div><form id=formDish class=row style="margin-top:8px;flex-wrap:wrap"><input name=dish_name placeholder="اسم" required style="flex:1"><input name=ip placeholder="IP" required style="flex:1"><input name=location placeholder="موقع" style="flex:1"><button class=btn-gold>إضافة</button></form></div><div id=dl>{rows}</div></div><script>window.editDish=function(id){{let c=document.getElementById("dish-"+id); let body=document.getElementById("editBody"); body.innerHTML="<input id=edn value=\\""+c.dataset.name+"\\"><input id=edi value=\\""+c.dataset.ip+"\\"><input id=edl value=\\""+c.dataset.loc+"\\"><button class=btn-gold onclick=\\"saveDish("+id+")\\">حفظ</button>"; document.getElementById("editModal").classList.add("show");}};window.saveDish=function(id){{fetch("/edit_dish/"+id,{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{dish_name:document.getElementById("edn").value,ip:document.getElementById("edi").value,location:document.getElementById("edl").value}})}}).then(r=>r.json()).then(j=>{{if(j.ok){{closeEditModal(); loadPage("dishes",true)}}}})}};window.quickPingD=function(id){{let ip=document.getElementById("dish-"+id).dataset.ip; loadPage("ping"); setTimeout(()=>{{let el=document.getElementById("pingIp"); if(el){{el.value=ip; doSinglePing()}}}},350)}};document.getElementById("formDish").addEventListener("submit",e=>{{e.preventDefault(); let fd=new FormData(e.target); fetch("/add_dish",{{method:"POST",body:fd}}).then(r=>r.json()).then(j=>{{if(j.ok){{e.target.reset(); loadPage("dishes",true)}} else alert(j.msg||"خطأ")}})}});</script>'
    if v=='map':
    ...
    if v=='ping':
        ...
        window.doSinglePing=async function(){
            ...
            out.textContent='جاري '+ip+'...';
            try{let r=await fetch('/api/ping?ip='+encodeURIComponent(ip),{cache:'no-store'}); let j=await r.json(); out.textContent=j.out;}catch(e){out.textContent='خطأ';}
        };
        window.doTcpPing=async function(){
            let ip=document.getElementById('pingIp').value.trim();
            let p=document.getElementById('pingPort').value||'80';
            let out=document.getElementById('pingResult');
            try{let r=await fetch('/api/ping_tcp?ip='+encodeURIComponent(ip)+'&port='+p); let j=await r.json(); out.textContent=j.out;}catch(e){out.textContent='خطأ';}
        };
...
.sidebar.active{{transform:none}}.sidebar a{{display:flex;gap:12px;padding:14px 16px;margin:7px 12px;color:#cbd5e1;text-decoration:none;border-radius:14px;background:#ffffff08;transition:transform .65s var(--ease), background .4s, color .4s; animation:slideIn .6s var(--ease) both}}
@keyframes slideIn{{from{{opacity:0;transform:translateX(30px)}}to{{opacity:1;transform:translateX(0)}}}}
.sidebar a:hover{{transform:translateX(-8px) scale(1.03); background:#ffffff14}}.sidebar a.active{{background:linear-gradient(90deg,#ffbe4d,#ffb020);color:#111;font-weight:900; transform:scale(1.04)}}.sidebar a:active{{transform:scale(.95)}}
...
.card{{background:{card_bg};color:{txt};padding:14px;border-radius:16px;margin-bottom:12px;border:1px solid {border}; animation:fadeUp .65s var(--ease) both; transition:transform .65s var(--ease), box-shadow .65s var(--ease)}}
...
.btn-gold{{background:linear-gradient(90deg,#ffbe4d,#ffb020);color:#111;padding:9px 14px;border:0;border-radius:11px;font-weight:800;cursor:pointer;transition:transform .65s var(--ease), filter .3s, box-shadow .3s}}.btn-gold:hover{{filter:brightness(1.08); box-shadow:0 4px 18px #ffb02044}}.btn-gold:active{{transform:scale(.88) !important}}
...
.rowlog{{display:flex;justify-content:space-between;padding:9px 10px;border-bottom:1px dashed #ffffff10;gap:8px}}.badge{{background:#ffbe4d;color:#111;padding:2px 8px;border-radius:8px;font-size:11px;font-weight:800}}.time{{color:#64748b;font-size:11px}}
.tower-card{{border:1px solid #ffbe4d22; overflow:hidden}}.tower-head{{display:flex;justify-content:space-between}}.tower-title{{font-size:17px;font-weight:900}}.coords{{color:#ffbe4d;font-size:11px}}.tower-body{{margin-top:10px;border-top:1px dashed #ffffff10;padding-top:10px}}.dish-list{{margin-top:8px;display:flex;flex-direction:column;gap:6px}}.dish-mini{{display:flex;justify-content:space-between;align-items:center;background:#ffffff06;padding:8px 10px;border-radius:10px;transition:transform .55s var(--ease)}}.dish-mini:hover{{transform:scale(1.01)}}
...
input,select{{padding:12px;border-radius:11px;border:1px solid {border};background:#ffffff07;color:{txt};transition:transform .45s var(--ease)}} input:focus{{transform:scale(1.01); border-color:#ffbe4d88; box-shadow:0 0 0 3px #ffbe4d22; outline:none}}
</style></head><body>
...
<div class=top><div class=row><span onclick="toggleSb()" style='font-size:24px;cursor:pointer;padding:6px 10px;background:#ffffff0a;border-radius:10px'>☰</span><input id=topsearch placeholder='بحث...' oninput="globalSearchTop(this.value)" style='width:44px;transition:all .5s var(--ease);background:#1f2937;border:1px solid #ffffff15;color:#fff;padding:8px 12px;border-radius:10px' onfocus="this.style.width='170px'" onblur="setTimeout(()=>this.style.width='44px',200)"></div><b>OMAIA <span style='color:#ffbe4d'>ISP</span></b><div class=row><button onclick="toggleLangFast()" style='background:#ffffff0a;color:#fff;border:1px solid #ffffff14;padding:8px 10px;border-radius:10px'>🌐</button><button onclick="toggleThemeFast()" style='background:#ffffff0a;color:#fff;border:1px solid #ffffff14;padding:8px 10px;border-radius:10px'>🌓</button></div></div>
...
document.getElementById('delYes').onclick=async()=>{{
 ...
 try{{let r=await fetch(window._delUrl,{{cache:'no-store'}}); let j=await r.json(); if(j.ok){{
   let el=document.getElementById('dish-'+window._delId) || document.getElementById('tower-'+window._delId) || document.getElementById('sub-'+window._delId) || document.getElementById('led-'+window._delId) || document.getElementById('net-'+window._delId);
...
window.toggleLangFast=async()=>{{await fetch('/toggle_lang',{{cache:'no-store'}}); location.reload();}};
window.toggleThemeFast=async()=>{{await fetch('/toggle_theme',{{cache:'no-store'}}); location.reload();}};
window.globalSearchTop=async q=>{{let box=document.getElementById('searchResults'); if(!q||q.length<2){{box.style.display='none'; return;}} try{{let r=await fetch('/api/search?q='+encodeURIComponent(q),{{cache:'no-store'}}); let d=await r.json(); if(!d.length){{box.style.display='none'; return;}} let h=''; d.forEach(x=>{{h+='<div onclick="loadPage(\\''+x.page+'\\');document.getElementById(\\'searchResults\\').style.display=\\'none\\'" style="padding:11px 12px;cursor:pointer;border-bottom:1px solid #ffffff08"><b>'+x.title+'</b><br><small style="color:#888">'+x.sub+'</small></div>';}}); box.innerHTML=h; box.style.display='block';}}catch(e){{}}}};
