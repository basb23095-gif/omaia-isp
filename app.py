...
# ===== Pool 25 اتصال - حل بطء تسجيل الدخول والإضافة =====
_pg_pool = None
...
def init_pg_pool():
    ...
    with _pg_pool_lock:
        ...
        try:
            ...
            print("[POOL] 25 connections ready - ULTRA FAST")
...
# ===== حل مشكلة السجل فاضي - يبين كل التعديلات =====
...
def init():
    ...
    if USE_PG:
        for idx in ["CREATE INDEX IF NOT EXISTS idx_dish_ips_ip ON dish_ips(ip)", "CREATE INDEX IF NOT EXISTS idx_ips_ip ON ips(ip)", "CREATE UNIQUE INDEX IF NOT EXISTS uq_dish_ips_ip ON dish_ips(ip)", "CREATE UNIQUE INDEX IF NOT EXISTS uq_ips_ip ON ips(ip)", "CREATE INDEX IF NOT EXISTS idx_logs_id ON logs(id DESC)"]:
            try: qexec(idx)
            except: pass
...
def public_ping():
    tbl=get_dish_table()
    return jsonify(ok=True, time=datetime.datetime.now().isoformat(), table=tbl, pool=bool(_pg_pool), fast=True)
...
def dash():
    ...
    return layout('<div class=card><div class="skeleton" style="height:28px;width:45%;border-radius:10px;margin-bottom:12px;background:linear-gradient(90deg,#1a2035 25%,#222b45 50%,#1a2035 75%);background-size:200% 100%;animation:shimmer 1s infinite"></div></div>', v, fast=True)
...
def ad():
    ...
    if ok:
        log_action(phone, 'إضافة/تعديل صحن', f'{name} {ip} - {loc}')
        with _cache_lock: _cache.pop('counts', None)
        return "ok"
    return "خطأ",400
...
def ed(i):
    ...
    dish_tbl=get_dish_table()
    ip=request.form.get('ip','').strip(); name=request.form.get('dish_name','').strip(); loc=request.form.get('location','').strip()
    ok=qexec(f"UPDATE {dish_tbl} SET dish_name=?,ip=?,location=? WHERE id=?", (name, ip, loc, i))
    if ok:
        log_action(session.get('phone'), 'تعديل صحن', f'ID {i} -> {ip} {name}')
...
def et(i):
    ...
    if ok: log_action(session.get('phone'), 'تعديل برج', f"ID {i} - {request.form.get('name','')}")
    return "ok" if ok else "خطأ", 200 if ok else 400
...
def page_content(v):
    ...
    if v=='home':
        ...
        if not logs: log_html=f"<div style='padding:12px;color:#64748b'>لا يوجد سجل بعد - <button class=btn-gold onclick=\"fetch('/api/seed_log',{{method:'POST'}}).then(()=>loadPage('home',true))\">🧪 اختبار السجل</button></div>"
        ...
        <div class=card style='margin-top:14px'><div style='display:flex;justify-content:space-between;flex-wrap:wrap'><h4>📊 {L('التقارير','Reports')} - <small style='color:#22c55e'>☁ {dish_tbl} • Pool 25 • فوري</small></h4><div style='display:flex;gap:8px'><a href='/api/export/dishes' class=btn-gold style='text-decoration:none;padding:8px 12px;background:linear-gradient(90deg,#22c55e,#16a34a);color:#fff'>📗 Excel</a><a href='/api/export/logs' class=btn-gold style='text-decoration:none;padding:8px 12px;background:linear-gradient(90deg,#8b5cf6,#7c3aed);color:#fff'>📜 سجل Excel</a></div></div></div>
        <div class=card><div style='display:flex;justify-content:space-between;align-items:center'><h4>📜 {L('آخر النشاطات','Recent')} - السجل شغال ✅ يبين كل التعديلات</h4><button class=btn-gold onclick="loadPage('logs')" style='padding:6px 12px'>عرض الكل</button></div>{log_html}</div></div>'''
    if v=='ping':
        ...
        <h3 style='margin:0'>📶 {L('بنج منفصل','Ping')} ⚡ ULTRA FAST</h3>
        <p style='color:#9ca3af;font-size:12px;margin:6px 0'>Pool 25 • {dish_tbl} • فوري</p>
    ...
    if v=='dishes':
        ...
        for r in rs:
            ...
            rows_html+=f'<div class="card anim" id="dish-{rid}" data-name="{dn}" data-ip="{ip}" data-loc="{loc}" style="display:flex;justify-content:space-between"><div><b>{dn}</b><br><a href="http://{ip}" target=_blank style="background:#000;color:#ffbe4d;padding:5px 10px;border-radius:8px;font-family:monospace;text-decoration:none">🌐 {ip}</a><br><small style="color:#888">{loc}</small><br><small style="color:#22c55e">☁ {dish_tbl} ⚡ فوري</small></div><div style="display:flex;flex-direction:column;gap:6px"><button class=btn-gold onclick="quickPingD({rid})" style="padding:7px 12px;background:linear-gradient(90deg,#22c55e,#16a34a);color:#fff">📶</button><div style="display:flex;gap:4px"><button class=btn-gold onclick="editDish({rid})" style="padding:7px 9px">✏</button><button class=btn-del onclick="askDel(\'/del_dish/{rid}\')" style="padding:7px 9px">🗑</button></div></div></div>'
        return f'''<div style='max-width:900px;margin:0 auto'><div class=card><div style='display:flex;justify-content:space-between;flex-wrap:wrap'><h3>📡 {L('الصحون','Dishes')} - {len(rs)} <small style='color:#22c55e'>☁ {dish_tbl} ⚡ فوري</small></h3><div style='display:flex;gap:6px'><button onclick="loadPage('ping')" class=btn-gold style='padding:7px 12px;background:linear-gradient(90deg,#22c55e,#16a34a);color:#fff'>📶 Ping</button><a href='/api/export/dishes' class=btn-gold style='text-decoration:none;padding:7px 12px'>📗 Excel</a></div></div><form data-ajax method=post action=/add_dish style='display:flex;gap:6px;flex-wrap:wrap;margin-top:10px'><input name=dish_name placeholder='اسم الصحن' required style='flex:1'><input name=ip placeholder='192.168.1.1' required style='flex:1'><input name=location placeholder='موقع' style='flex:1'><button class=btn-gold>➕ حفظ فوري ⚡</button></form><input id=searchBox placeholder='🔍 بحث...' oninput="searchDishes(this.value)" style='margin-top:10px;width:100%;padding:12px;border-radius:12px;background:#0f1424;border:1px solid #ffffff18'></div><div id=dl>{rows_html}</div></div><script>
        function editDish(id){{let c=document.getElementById('dish-'+id);document.getElementById('editModal').classList.add('show');document.getElementById('editBody').innerHTML='<input id=edit_dish_name value="'+c.dataset.name+'" style="width:100%;padding:12px;margin:4px 0"><input id=edit_ip value="'+c.dataset.ip+'" style="width:100%;padding:12px;margin:4px 0"><input id=edit_loc value="'+c.dataset.loc+'" style="width:100%;padding:12px;margin:4px 0"><button onclick="saveDish('+id+')" class=btn-gold style="width:100%;padding:12px">💾 حفظ فوري</button>';}}
    ...
    if v=='towers':
        ...
        for r in rs: rows+=f"<div class='card anim' id='tower-{r['id']}' data-name='{esc(r['name'])}' data-area='{esc(r['area'] or '')}' data-lat='{r.get('lat') or 0}' data-lng='{r.get('lng') or 0}'><div style='display:flex;justify-content:space-between'><div><b>🗼 {esc(r['name'])}</b><br><small>{esc(r['area'] or '')}</small><br><small style='color:#ffbe4d'>📍 {r.get('lat')} , {r.get('lng')}</small></div><div style='display:flex;gap:5px'><button class=btn-gold onclick=\"openEditTower({r['id']})\" style='padding:8px 10px'>✏</button><button class=btn-del onclick=\"askDel('/del_tower/{r['id']}')\" style='padding:8px 10px'>🗑</button></div></div></div>"
    ...
    if v=='logs':
        ...
        rows="".join([f"<div class='card anim' style='font-size:13px;border-right:4px solid #ffbe4d'><div style='display:flex;justify-content:space-between'><div><b style='color:#ffbe4d'>{esc(r.get('user_phone',''))}</b> <span style='background:{'#22c55e' if 'إضافة' in r.get('action','') else '#0ea5e9' if 'تعديل' in r.get('action','') else '#ef4444' if 'حذف' in r.get('action','') else '#ffbe4d'};color:#fff;padding:2px 8px;border-radius:6px;font-size:11px'>{esc(r.get('action',''))}</span> <small style='color:#cbd5e1'>{esc(r.get('detail',''))}</small></div><small style='color:#64748b'>{esc(r.get('time',''))}</small></div></div>" for r in rs])
        if not rs: rows="<div class=card style='text-align:center;padding:20px;color:#64748b'>📭 السجل فاضي - اضف أي شي ورح يظهر هنا فوراً<br><button class=btn-gold onclick=\"fetch('/api/seed_log',{method:'POST'}).then(()=>loadPage('logs',true))\" style='margin-top:10px'>🧪 اختبار السجل</button></div>"
        return f"<div style='max-width:900px;margin:0 auto'><div class=card style='display:flex;justify-content:space-between;align-items:center'><h3>📜 السجل - شغال ✅ يبين كل التعديلات ({len(rs)})</h3><div style='display:flex;gap:6px'><a href='/api/export/logs' class=btn-gold style='text-decoration:none;padding:7px 12px;background:#22c55e;color:#fff'>📗 Excel</a><button onclick=\"if(confirm('مسح السجل؟')){{fetch('/api/clear_logs',{method:'POST'}).then(()=>loadPage('logs',true))}}\" class=btn-del style='padding:7px 12px'>🗑 مسح</button></div></div>{rows}</div>"
    if v=='network':
        dish_tbl=get_dish_table()
        ...
        return f'''<div style='max-width:800px;margin:0 auto'><div class=card style='background:linear-gradient(135deg,#0f172a,#1e293b);border:1px solid #ffbe4d33'><h3>📊 حالة الشبكة LIVE - ☁ {dish_tbl} ⚡ فوري</h3><div style='display:flex;gap:8px;margin-top:8px'><button class=btn-gold onclick='checkAll()' style='flex:1;background:linear-gradient(90deg,#22c55e,#16a34a);color:#fff;padding:12px'>🚀 فحص الكل</button><button class=btn-gold onclick="loadPage('ping')" style='flex:1'>📶 Ping</button></div><div id=summary style='margin-top:10px;font-weight:800'></div></div>{rows}<script>
    ...
    if v=='map':
        ...
        <div style='margin-top:6px;font-size:11px;color:#6b7280;display:flex;gap:12px;flex-wrap:wrap'><span>💡 اضغط على الخريطة لتحديد موقع</span><span id=coordsLabel style='color:#ffbe4d'>📍 -</span></div>
        </div><script>
        let _towers={tj_json};
        ...
        window.doMapSearch=function(){{
          ...
          if(f && _map){{_map.flyTo([f.lat,f.lng],17,{{duration:1}}); L.popup().setLatLng([f.lat,f.lng]).setContent('<b>🗼 '+f.name+'</b>').openOn(_map);}}
        }}
        ...
        window.enableAddPoint=function(){{addPointMode=!addPointMode; let b=document.getElementById('addPointBtn'); b.textContent=addPointMode?'✅ اضغط على الخريطة':'➕ نقطة'; b.style.background=addPointMode?'linear-gradient(90deg,#22c55e,#16a34a)':'linear-gradient(90deg,#f59e0b,#d97706)'; if(addPointMode){{measureMode=false; _map.getContainer().style.cursor='crosshair';}}else{{_map.getContainer().style.cursor='';}}}}
        ...
        setTimeout(()=>{{
          ...
          _towers.forEach(t=>{{L.marker([t.lat,t.lng],{{draggable:true}}).addTo(_map).bindPopup('<b>'+t.name+'</b><br>'+t.area);}});
          _map.on('click',e=>{{
            ...
            if(addPointMode){{
              ...
              L.popup().setLatLng(e.latlng).setContent('<div style="min-width:200px;text-align:right"><b>➕ نقطة جديدة</b><br><small style="color:#ffbe4d">'+lat+','+lng+'</small><br><input id="newPointName" placeholder="اسم" style="width:100%;margin:6px 0;padding:8px;border-radius:8px"><input id="newPointArea" placeholder="منطقة" style="width:100%;margin:4px 0;padding:8px;border-radius:8px"><button onclick="saveNewPoint('+lat+','+lng+')" style="width:100%;background:linear-gradient(90deg,#ffbe4d,#ffb020);border:0;padding:9px;border-radius:8px;font-weight:800;margin-top:6px">💾 حفظ</button></div>').openOn(_map);
            }}
          }});
...
def layout(c, v='home', fast=False):
    ...
    cur_user=qone("SELECT * FROM users WHERE phone=?", (session.get('phone') or '',)) or {}; role=(cur_user.get('role') or 'tech') if cur_user else session.get('role','tech')
    req_lang=session.get('lang','ar'); is_rtl=req_lang=='ar'
    def L(ar,en): return ar if is_rtl else en
    username_display=esc(cur_user.get('username') or cur_user.get('phone') or session.get('phone') or '') if cur_user else esc(session.get('phone') or '')
...
.sidebar{{position:fixed;top:0;width:285px;height:100%;background:linear-gradient(180deg,#0f172a 0%,#070e22 100%);color:#fff;z-index:1002;padding-top:70px;{sidebar_pos}transition:transform .32s cubic-bezier(.4,0,.2,1);overflow-y:auto;box-shadow:10px 0 40px #0008;border-left:1px solid #ffffff0f}}
...
<div style='padding:0 18px 10px;border-bottom:1px solid #ffffff0a;margin-bottom:8px'><div style='font-weight:900;font-size:17px'>OMAIA <span style='color:#ffbe4d'>ISP</span> <small style='color:#22c55e'>⚡ ULTRA</small></div><small style='color:#64748b'>{username_display} • {role} • {get_dish_table()} • Pool 25</small><br><small style='color:#22c55e'>✅ فوري • سجل يبين التعديلات • دخول فوري</small></div>
<a href="javascript:loadPage('home')" id=nav-home onmouseenter="prefetchPage('home')">🏠 {L('الرئيسية','Home')}</a>
<a href="javascript:loadPage('ping')" id=nav-ping onmouseenter="prefetchPage('ping')" style='background:linear-gradient(90deg,#22c55e18,#16a34a18);border:1px solid #22c55e33'>📶 {L('بنج منفصل','Ping')} <span style='background:#22c55e;color:#fff;padding:2px 6px;border-radius:6px;font-size:9px'>⚡ فوري</span></a>
<a href="javascript:loadPage('network')" id=nav-network onmouseenter="prefetchPage('network')">📊 {L('حالة الشبكة','Network')} LIVE</a>
...
<a href="javascript:loadPage('logs')" id=nav-logs onmouseenter="prefetchPage('logs')">📜 {L('السجل','Logs')} <span style='background:#22c55e;color:#fff;padding:2px 6px;border-radius:6px;font-size:9px'>شغال</span></a>
<a href="javascript:loadPage('map')" id=nav-map onmouseenter="prefetchPage('map')">🗺 {L('الخريطة الحية','Live Map')} HD</a>
...
<input id=topsearch placeholder='🔍 {L('بحث','Search')}...' oninput="globalSearchTop(this.value)" style='background:#1f2937;border:1px solid #ffffff15;color:#fff;padding:9px 14px;border-radius:12px;width:42px;font-size:13px;transition:all .28s' onfocus="this.style.width='200px'" onblur="setTimeout(()=>{{this.style.width='42px'; let b=document.getElementById('searchResults'); if(b) b.style.display='none';}},250)">
</div>
<div style='font-weight:900;font-size:16px'>OMAIA <span style='color:#ffbe4d'>ISP</span> <span style='color:#22c55e;font-size:10px'>⚡ ULTRA • Pool 25</span></div>
<div style='display:flex;gap:8px;align-items:center'>
...
<div id=searchResults style='position:fixed;top:66px;right:12px;max-width:480px;width:92%;background:linear-gradient(180deg,#1e2433,#171e2f);border:1px solid #ffffff15;border-radius:14px;z-index:1500;display:none;max-height:60vh;overflow:auto;box-shadow:0 16px 40px #000a'></div>
...
<div id=delModal><div id=delBox><div style='font-size:42px;text-align:center'>🗑</div><h3 style='text-align:center;margin:8px 0'>تأكيد الحذف الفوري؟</h3><p style='text-align:center;color:#94a3b8;font-size:13px'>سيتم الحذف فورياً وسيظهر في السجل</p><div style='display:flex;gap:10px;margin-top:14px'><button onclick="closeDel()" style='flex:1;padding:12px;border-radius:12px;border:1px solid {border};background:transparent;color:{txt};cursor:pointer'>تراجع</button><button id=delYes style='flex:1;padding:12px;border-radius:12px;background:linear-gradient(90deg,#ef4444,#dc2626);color:#fff;border:0;cursor:pointer;font-weight:800'>حذف فوري</button></div></div></div>
<div id=editModal><div id=editBox><div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:14px'><h3 id=editTitle style='margin:0'>✏ تعديل فوري</h3><button onclick="closeEditModal()" style='background:#ffffff12;border:0;color:{txt};width:32px;height:32px;border-radius:50%;cursor:pointer'>✕</button></div><div id=editBody></div></div></div>
<script src='https://unpkg.com/leaflet@1.9.4/dist/leaflet.js'></script>
<script>
let cur='{v}'; let lang=localStorage.getItem('omaia_lang')||'{req_lang}';
function toggleSb(f){{let sb=document.getElementById('sb'),ov=document.getElementById('overlay'); let o=f!==undefined?f:!sb.classList.contains('active'); sb.classList.toggle('active',o); ov.classList.toggle('show',o); if(o){{ov.style.display='block'; setTimeout(()=>ov.style.opacity='1',10);}} else {{ov.style.opacity='0'; setTimeout(()=>ov.style.display='none',300);}}}}
let pageCache={{}}; try{{pageCache=JSON.parse(localStorage.getItem('omaia_ultra_full_v1')||'{{}}');}}catch(e){{pageCache={{}};}}
function saveCache(){{try{{localStorage.setItem('omaia_ultra_full_v1',JSON.stringify(pageCache));}}catch(e){{}}}}
...
async function loadPage(v,force=false,push=true){{
  ...
  mn.innerHTML='<div class=card><div class="skeleton" style="height:20px;width:40%;border-radius:8px;margin-bottom:10px"></div><div class="skeleton" style="height:14px;border-radius:6px;margin-bottom:6px"></div></div>';
...
function bind(){{
  document.querySelectorAll('form[data-ajax]').forEach(f=>{{
    ...
    f.onsubmit=async e=>{{
      e.preventDefault(); let btn=f.querySelector('button'); let old=btn?btn.innerHTML:''; if(btn){{btn.innerHTML='⏳ فوري...'; btn.disabled=true;}}
...
window.logoutFast=async function(){{try{{await fetch('/api/logout',{{method:'POST'}});}}catch(e){{}} try{{localStorage.removeItem('omaia_cache_v3'); localStorage.removeItem('omaia_ultra_full_v1');}}catch(e){{}} location.replace('/login');}};
...
bind(); execScripts();
setTimeout(()=>{{let pages=['home','dishes','towers','subs','ledger','logs','ping','map','network']; let i=0; function next(){{if(i>=pages.length) return; let p=pages[i++]; if(!pageCache[p]){{fetch('/api/page?v='+p+'&lang='+lang,{{cache:'no-store'}}).then(r=>r.text()).then(h=>{{pageCache[p]=h; saveCache(); setTimeout(next,200);}}).catch(()=>setTimeout(next,200));}} else setTimeout(next,80);}} if('requestIdleCallback' in window) requestIdleCallback(next,{{timeout:2000}}); else next();}},1200);
