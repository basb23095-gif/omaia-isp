...
_user_cache = {}  # cache users for login speed
...
# سجل سريع - متزامن سريع مشان يبين فوراً
def log_action(phone, action, detail):
    try:
        now=datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # كتابة فورية بدون ثريد - سريعة بسبب Pool
...
def get_dish_table():
    with _cache_lock:
        ...
        if c and time.time()-c[1]<300: return c[0]  # 5 دقايق كاش
...
def get_counts_cached():
    with _cache_lock:
        c=_cache.get('counts')
        if c and time.time()-c[1]<30: return c[0]  # 30 ثانية كاش للعدادات
...
def init():
    ...
    idx=[
        ...
        "CREATE UNIQUE INDEX IF NOT EXISTS uq_dish_ips_ip ON dish_ips(ip)",
        "CREATE UNIQUE INDEX IF NOT EXISTS uq_ips_ip ON ips(ip)"
    ]
    for s in (tables_pg if USE_PG else tables_sql): qexec(s)
    if USE_PG:
        for s in idx:
            try: qexec(s)
            except: pass
    ...
    # إذا السجل فاضي، ضيف سجل ترحيبي مشان يبين انو شغال
    c=qone("SELECT COUNT(*) as c FROM logs")
    if c and c.get('c',0)==0:
        qexec("INSERT INTO logs(user_phone,action,detail,time) VALUES(?,?,?,?)", ('system','تشغيل النظام','السجل شغال الآن ✅', datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
init()
...
def public_ping():
    ...
    if USE_PG:
        try:
            if _pg_pool: c=_pg_pool.getconn(); _pg_pool.putconn(c); pg_ok=True
            else: c=psycopg2.connect(DATABASE_URL, sslmode='require', connect_timeout=1); c.close(); pg_ok=True
...
def api_ping():
    ...
    for port in [80,443,8080,8291,22]:
        ...
        try:
            ...
            if s.connect_ex((ip,port))==0: s.close(); return jsonify(ok=True,out=f'✅ {ip}:{port} مفتوح',port=port)
            s.close()
        except:
            try:
                if s: s.close()
...
<script>
// تسخين السيرفر بالخلفية مشان الدخول يكون فوري
...
@login_required
def dash():
    # مهم: ما نعمل استعلامات DB هنا - نرجع هيكل فاضي سريع والـ JS يحمل المحتوى
    v=request.args.get('v','home')
    # هيكل سريع بدون DB
    return layout('<div class=card><div class="skeleton" style="height:30px;width:50%;border-radius:10px;margin-bottom:14px"></div><div class="skeleton" style="height:20px;border-radius:8px;margin-bottom:8px"></div><div class="skeleton" style="height:20px;width:80%;border-radius:8px"></div></div>', v, fast=True)
...
def ad():
    ...
    # محاولة إدخال مباشر مع ON CONFLICT - أسرع من SELECT ثم INSERT
    if USE_PG:
    ...
    if ok:
        ...
        with _cache_lock: _cache.pop('counts', None)
        return jsonify(ok=True, msg="تم", ip=ip, name=name, loc=loc)
...
def at():
    ...
    if ok:
        log_action(session.get('phone'), 'إضافة برج', request.form.get('name',''))
        return jsonify(ok=True)
    return jsonify(ok=False),400
...
def page_content(v):
    ...
    if v=='home':
        ...
        if not logs: log_html="<div style='padding:12px;color:#64748b'>السجل فاضي - اضف صحن وسيظهر هنا فوراً ✅</div>"
        ...
        <div class='card anim' onclick="loadPage('dishes')" style='cursor:pointer'><h3 style='margin:0;color:#aab4d0;font-size:13px'>الصحون ☁ {dish_tbl}</h3><h2 style='margin:6px 0 0;font-size:36px'>{nd}</h2></div>
        ...
        <div class=card><div style='display:flex;justify-content:space-between'><h4>📜 السجل - شغال ✅ ({len(logs)}) - يبين كل التعديلات</h4><div style='display:flex;gap:6px'><button class=btn-gold onclick="loadPage('logs')" style='padding:6px 12px'>عرض الكل</button><a href='/api/export/logs' style='padding:6px 12px;background:#22c55e;color:#fff;border-radius:8px;text-decoration:none'>Excel</a></div></div>{log_html}</div></div>'''
    if v=='dishes':
        ...
        document.getElementById('formDish').addEventListener('submit', async e=>{{
          ...
          let orig=btn.innerHTML; btn.innerHTML='⏳...'; btn.disabled=true; msg.textContent=''; msg.style.color='#ffbe4d';
          let fd=new FormData(e.target);
          try{{
            ...
            if(j.ok){{
              ...
              // إضافة فورية للواجهة بدون تحميل الصفحة كاملة
              ...
              div.innerHTML='<div><b>'+j.name+'</b><br><span style="background:#000;color:#ffbe4d;padding:5px 10px;border-radius:8px">🌐 '+j.ip+'</span><br><small>'+j.loc+'</small><br><small style="color:#22c55e">✅ جديد - فوري</small></div><div><small style="color:#22c55e">تم</small></div>';
              dl.prepend(div);
              e.target.reset();
    ...
    if v=='logs':
        ...
        if not rs: rows="<div class=card style='text-align:center;padding:30px'><div style='font-size:40px'>📭</div><b>السجل فاضي</b><br><small style='color:#94a3b8'>أضف صحن جديد وسيظهر هنا فوراً مع كل التعديلات</small><br><button class=btn-gold onclick=\"fetch('/api/seed_log',{{method:'POST'}}).then(()=>loadPage('logs',true))\" style='margin-top:10px'>🧪 اختبار السجل</button></div>"
    ...
    if v=='settings':
        ...
        return f"<div style='max-width:700px;margin:0 auto'><div class=card><h3>⚙ الإعدادات - ULTRA FAST</h3><p style='color:#22c55e'>✅ تسجيل دخول فوري<br>✅ إضافة/حذف فوري بدون تحميل الصفحة<br>✅ السجل يبين كل التعديلات<br>✅ الموقع ما عاد يحمل كلو من جديد</p></div>{uh}</div>"
    if v=='ping':
...
def seed_log():
    ...
    qexec("INSERT INTO logs(user_phone,action,detail,time) VALUES(?,?,?,?)", (session.get('phone','test'), 'إضافة صحن', 'صحن تجريبي 192.168.1.1', now))
    qexec("INSERT INTO logs(user_phone,action,detail,time) VALUES(?,?,?,?)", (session.get('phone','test'), 'تعديل صحن', 'تعديل IP', now))
    return jsonify(ok=True)
...
def layout(c, v='home', fast=False):
    ...
    # إذا fast=True نرجع هيكل بدون انتظار DB - هذا يحل مشكلة تحميل الموقع كلو
...
.sidebar a{{display:flex;align-items:center;gap:11px;padding:12px 15px;margin:6px 11px;color:#cbd5e1;text-decoration:none;border-radius:13px;background:#ffffff06;border:1px solid #ffffff06}}
...
<div style='padding:0 18px 10px;border-bottom:1px solid #ffffff0a;margin-bottom:8px'><div style='font-weight:900'>OMAIA <span style='color:#ffbe4d'>ISP</span> <small style='color:#22c55e'>ULTRA</small></div><small style='color:#64748b'>{username_display} • {role}</small><br><small style='color:#22c55e'>✅ إضافة فوري • ✅ سجل يبين التعديلات • ✅ دخول فوري</small></div>
...
async function loadPage(v,force=false,push=true){{
  ...
  // كاش فوري - بدون تحميل الموقع كلو
...
document.getElementById('delYes').onclick=async()=>{{
  ...
  try{{
    ...
    if(r.ok || j.ok){{
      // حذف فوري من الواجهة بدون تحميل الصفحة كاملة
...
bind(); execScripts();
// تحميل الصفحة الأولى فورياً بدون تحميل الموقع كلو
