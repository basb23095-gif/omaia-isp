...
.card{background:rgba(34,43,69,0.9);backdrop-filter:blur(14px);border:1px solid #ffffff18;padding:26px;border-radius:22px;width:92%;max-width:380px}
...
def ed(i):
    ...
    if ok:
        add_log(session.get('phone'),'تعديل صحن',"ID "+str(i)+" -> "+ip+" "+name)
    return jsonify(ok=ok)
...
def al():
    ...
    if ok:
        add_log(session.get('phone'),'إضافة حساب',str(request.form.get('name',''))+" "+str(amt))
    return jsonify(ok=ok)
...
def page_content(v):
    ...
    if v=='home':
        ...
        html_out += "</div><div class=card style='margin-top:14px'><div style='display:flex;justify-content:space-between'><h4>آخر النشاطات</h4><button class=btn-gold onclick=\"loadPage('logs')\">عرض الكل</button></div><div style='margin-top:10px'>"+log_html+"</div></div></div>"
        return html_out
...
"""
    ...
    if v=='dishes':
        ...
        for r in rs:
            ...
            rid=r['id']
            rows_html += '<div class="card dish-card card-anim" id="dish-'+str(rid)+'" data-name="'+dn+'" data-ip="'+ip+'" data-loc="'+loc+'" style="display:flex;justify-content:space-between;align-items:center"><div><b>'+dn+'</b><br><a href="http://'+ip+'" target=_blank style="background:#000;color:#ffbe4d;padding:4px 10px;border-radius:8px;font-family:monospace;text-decoration:none;display:inline-block;margin:4px 0">'+ip+'</a><br><small style="color:#888">'+loc+'</small></div><div style="display:flex;flex-direction:column;gap:6px"><button class="btn-gold icon-anim" onclick="quickPingD('+str(rid)+')" style="padding:7px 12px;background:#22c55e;color:#fff">Ping</button><div style="display:flex;gap:4px"><button class="btn-gold icon-anim" onclick="editDish('+str(rid)+')" style="padding:7px 9px">تعديل</button><button class="btn-del icon-anim" onclick="askDel(\'/del_dish/'+str(rid)+'\','+str(rid)+',\'dish\')" style="padding:7px 9px">حذف</button></div></div></div>'
        return """
...
window.saveDish=async function(id){
  ...
  else { alert(j.msg||'ممنوع'); b.textContent='حفظ'; b.disabled=false; }
};
...
document.getElementById('formDish').addEventListener('submit', async e=>{
  e.preventDefault();
  let btn=document.getElementById('btnAddDish'); btn.textContent='جاري...'; btn.disabled=true;
  let r=await fetch('/add_dish',{method:'POST',body:new FormData(e.target)});
  let j=await r.json();
  if(j.ok){ e.target.reset(); loadPage('dishes',true); }
  else { alert(j.msg||'خطأ'); btn.textContent='إضافة'; btn.disabled=false; }
});
</script>
"""
...
setTimeout(()=>{
  ...
  window.saveNewPoint=async function(lat,lng){
    ...
    let j=await r.json();
    if(j.ok){ _map.closePopup(); loadPage('towers',true); }
  };
},400);
</script>
"""
    ...
    if v=='settings':
        ...
        return "<div style='max-width:800px;margin:0 auto'><div class=card><h3>كلمة السر</h3><form id=formPass style='display:flex;gap:8px'><input name=newpass type=password placeholder='جديدة' required style='flex:1'><button class=\"btn-gold icon-anim\">حفظ</button></form></div><div style='display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px'><div class=card style='text-align:center'><h4>اللغة والثيم</h4><button onclick=\"toggleLangNoReload()\" class=\"btn-gold icon-anim\" style='width:100%;padding:12px;background:#1f2937;color:#fff'>تغيير اللغة</button><button onclick=\"toggleThemeNoReload()\" class=\"btn-gold icon-anim\" style='width:100%;padding:12px;margin-top:8px'>ثيم</button></div><div class=card><h4>إضافة يوزر</h4><form id=formUser style='display:flex;flex-direction:column;gap:8px'><input name=user_field placeholder='رقم / يوزر' required style='padding:12px;background:#0f1424;border:1px solid #ffffff20;border-radius:12px;color:#fff'><input name=password type=password placeholder='كلمة السر' required style='padding:12px;background:#0f1424;border:1px solid #ffffff20;border-radius:12px;color:#fff'><select name=role style='padding:12px;background:#0f1424;border:1px solid #ffffff20;border-radius:12px;color:#fff'><option value=tech>فني</option><option value=manager>مدير</option></select><button class=\"btn-gold icon-anim\" style='padding:12px'>إضافة</button></form></div></div><div class=card><h4>تصدير</h4><div style='display:flex;gap:8px;flex-wrap:wrap'><a href='/api/export/users' class=btn-gold style='text-decoration:none;padding:8px 12px;background:#22c55e;color:#fff;border-radius:8px'>يوزرات</a><a href='/api/export/dishes' class=btn-gold style='text-decoration:none;padding:8px 12px;background:#0ea5e9;color:#fff;border-radius:8px'>صحون</a><a href='/api/export/towers' class=btn-gold style='text-decoration:none;padding:8px 12px;background:#ffbe4d;color:#111;border-radius:8px'>أبراج</a><a href='/api/export/logs' class=btn-gold style='text-decoration:none;padding:8px 12px;background:#8b5cf6;color:#fff;border-radius:8px'>سجل</a></div></div><div style='display:grid;gap:8px'>"+uh+"</div></div><script>window.openEditUser=function(ph){ let c=document.getElementById('user-'+ph); let body=document.getElementById('editBody'); body.innerHTML='<input id=edit_u_field value=\"'+c.dataset.phone+'\" style=\"width:100%;padding:12px\"><input id=edit_u_pass type=\"password\" placeholder=\"كلمة سر جديدة\" style=\"width:100%;padding:12px;margin-top:8px\"><select id=edit_u_role style=\"width:100%;padding:12px;margin-top:8px\"><option value=\"tech\" '+(c.dataset.role=='tech'?'selected':'')+'>فني</option><option value=\"manager\" '+(c.dataset.role=='manager'?'selected':'')+'>مدير</option></select><button onclick=\"saveUser(\\''+ph+'\\')\" class=btn-gold style=\"width:100%;padding:14px;margin-top:12px\">حفظ</button>'; document.getElementById('editModal').classList.add('show'); }; window.saveUser=async function(oldPh){ let ff=document.getElementById('edit_u_field').value.trim(); let pw=document.getElementById('edit_u_pass').value; let ro=document.getElementById('edit_u_role').value; if(!ff){ alert('مطلوب'); return; } let fd=new URLSearchParams(); fd.append('old_phone',oldPh); fd.append('phone',ff); fd.append('username',ff); fd.append('role',ro); if(pw.trim()!=''){ fd.append('password',pw.trim()); } let r=await fetch('/edit_user',{method:'POST',body:fd}); let j=await r.json(); if(!j.ok) alert(j.msg||'خطأ'); else { closeEditModal(); loadPage('settings',true); } }; document.getElementById('formPass').addEventListener('submit', async e=>{ e.preventDefault(); let fd=new FormData(e.target); let r=await fetch('/change_pass',{method:'POST',body:fd}); let j=await r.json(); if(j.ok){ e.target.reset(); } else alert(j.msg||'خطأ'); }); document.getElementById('formUser').addEventListener('submit', async e=>{ e.preventDefault(); let r=await fetch('/add_user',{method:'POST',body:new FormData(e.target)}); let j=await r.json(); if(j.ok){ e.target.reset(); loadPage('settings',true); } else alert(j.msg||'خطأ'); });</script>"
...
.card:hover{transform:translateY(-3px);box-shadow:0 10px 30px #0004}
...
.icon-anim{transition:all .42s cubic-bezier(0.34, 1.56, 0.64, 1);display:inline-block} .icon-anim:hover{transform:scale(1.18) rotate(3deg)} .icon-anim:active{transform:scale(0.82)}
...
.btn-del{background:linear-gradient(90deg,#ef4444,#dc2626);color:#fff;padding:8px 13px;border:0;border-radius:12px;cursor:pointer;transition:all .42s} .btn-del:hover{transform:scale(1.06)} .btn-del:active{transform:scale(0.88)}
...
if __name__=='__main__':
    ...
    print("OMAIA ISP FIXED V2 - running on 0.0.0.0:"+str(port))
