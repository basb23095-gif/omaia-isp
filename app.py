...
def u3():
    ...
    if f:
        ...
        for _, r in df.iterrows():
            ex(c, "INSERT INTO ledger(date,sub,amount,currency,note)VALUES(?,?,?,?,?)", (str(r.get('date', '')), str(r.get('sub', '')), float(r.get('amount', 0) or 0), str(r.get('currency', 'USD')), str(r.get('note', ''))))
        c.commit()
        close(c)
    return redirect('/dash?view=ledger')
...
def a4():
    ...
    if i:
        try:
            ex(c, "INSERT INTO users(phone,username,password,role,active)VALUES(?,?,?,?,1)", (i, i, request.form.get('password'), 'tech'))
            c.commit()
        except:
            pass
    close(c)
...
def e4(p):
    ...
    if request.method == 'POST':
        ...
        ex(c, "UPDATE users SET phone=?,username=?,password=? WHERE phone=?", (ni, ni, request.form.get('password'), p))
        c.commit()
        close(c)
        return redirect('/dash?view=settings')
...
def t4(p):
    ...
    if row:
        u = dict(row)
        ex(c, "UPDATE users SET active=? WHERE phone=?", (0 if u['active'] else 1, p))
        c.commit()
    close(c)
    return redirect('/dash?view=settings')

# تم إصلاح شرط الإقلاع هنا ليعمل السيرفر بنجاح دون الخروج بـ status 1
