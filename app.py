...
.icard{{border-radius:12px;padding:10px 4px;color:#fff;text-align:center;display:flex;flex-direction:column;justify-content:center;min-height:78px}}
...
function loadView(v,force){{sb.classList.add('hide');curV=v;history.replaceState(null,'','#'+v);if(cache[v]&&!force){{mn.innerHTML=cache[v];bind();return}}if(!force){{mn.innerHTML='<div class=skel></div><div class=skel></div>'}}fetch('/api/view?v='+v,{{headers:{{'X-Requested-With':'fetch'}}}}).then(r=>r.text()).then(h=>{{cache[v]=h;if(curV==v){{mn.innerHTML=h;bind()}}}}).catch(e=>{{}})}}
function bind(){{var scripts=mn.querySelectorAll('script');scripts.forEach(s=>{{var n=document.createElement('script');if(s.src){{n.src=s.src;document.head.appendChild(n);setTimeout(()=>{{var n2=document.createElement('script');n2.textContent=s.textContent||'';document.body.appendChild(n2);}},400);}}else{{n.textContent=s.textContent;document.body.appendChild(n);}}s.remove();}});}}
...
def allow_add(key, val):
    now=time.time();val=(val or "").strip()
...
def edit_dish(i):
    ...
    if request.method=='POST':
        f=request.form
        ex(c,"UPDATE dish_ips SET ip=?,location=?,area=?,tower=?,lat=?,lng=? WHERE id=?",(f.get('ip'),f.get('location'),f.get('area'),f.get('tower'),fnum(f.get('lat')),fnum(f.get('lng')),i))
        safe_commit(c);cc(c);return redirect('/dash#dishes')
...
def edit_tower(i):
    ...
    if request.method=='POST':
        f=request.form
        ex(c,"UPDATE towers SET name=?,area=?,location=?,owner=?,lat=?,lng=? WHERE id=?",(f.get('name'),f.get('area'),f.get('location'),f.get('owner'),fnum(f.get('lat')),fnum(f.get('lng')),i))
        safe_commit(c);cc(c);return redirect('/dash#towers')
...
def ch():
    ...
    amt=fnum(request.form.get('amount'));cur=request.form.get('currency','usd');cust_name=request.form.get('cust_name','')
...
def edit_ledger(i):
    c=db()
    if request.method=='POST':
        ex(c,"UPDATE ledger SET note=?,usd=?,syr=? WHERE id=?",(request.form.get('note'),fnum(request.form.get('usd')),fnum(request.form.get('syr')),i));safe_commit(c);cc(c);return redirect('/dash#ledger')
