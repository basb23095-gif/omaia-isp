...
import os, datetime, html, subprocess, platform, ipaddress, math
try:
...
if not app.secret_key:
    raise RuntimeError("SECRET_KEY env required")
...
def init():
    ss=["CREATE TABLE IF NOT EXISTS users(phone TEXT PRIMARY KEY,password TEXT,role TEXT,active INT DEFAULT 1,username TEXT)",
        ...
        "CREATE TABLE IF NOT EXISTS ledger(id INTEGER PRIMARY KEY AUTOINCREMENT,sub_id INT,amount REAL,typ TEXT,dt TEXT,note TEXT,currency TEXT DEFAULT 'SYP',name TEXT)",
...
def is_internal_ip(ip):
    try:
        ...
        # امنع loopback والـ multicast والـ reserved
        if not ip_o.is_private:
            return False
        # امنع 0.0.0.0 و 127.0.0.1
        if ip_o.is_loopback or ip_o.is_multicast or ip_o.is_reserved:
            return False
        return True
    except: return False
...
def page_content(v):
    ...
    if v=='home':
        ...
        <div class=card><h3>Ping Test - الشبكة الداخلية فقط</h3><div style="display:flex;gap:6px"><input id=ping_ip placeholder="192.168.1.1"><button onclick="doPing()">بينغ</button></div><pre id=ping_out style="background:#000;color:#0f0;padding:8px;border-radius:8px;min-height:60px"></pre></div>
        <script>async function doPing(){var i=document.getElementById("ping_ip").value;document.getElementById("ping_out").textContent="...";var r=await fetch("/api/ping?ip="+encodeURIComponent(i));var j=await r.json();document.getElementById("ping_out").textContent=j.out}</script>"""
        return h
    if v=='subs':
        ...
        for r in rs: h+="<div class=card grid-2><div><b>"+esc(r['name'])+"</b><br><small>"+esc(r['phone'])+"</small></div><div><a href=/del_sub/"+str(r['id'])+" data-del "+dis+" class='btn btn-danger'>حذف</a></div></div>"
        return h
    if v=='dishes':
        rs=qall("SELECT * FROM dish_ips ORDER BY id DESC LIMIT 200")
        h="<div class=card><h3>الصحون</h3><form data-ajax method=post action=/add_dish><input name=dish_name required placeholder='اسم الطبق'><input name=ip required placeholder='IP مثل 192.168.1.10'><input name=location placeholder='الموقع'><button>اضافة</button></form></div><div class=grid-2>"
        for r in rs:
            h+="<div class='card small'><b>"+esc(r.get('dish_name') or '')+"</b> - "+esc(r.get('ip') or '')+"<br><small>"+esc(r.get('location') or '')+"</small><br><button class=btn onclick='pingDish(""+esc(r.get('ip') or '')+"")'>بينغ</button> <a href=/del_dish/"+str(r['id'])+" data-del "+dis+" class='btn btn-danger'>حذف</a></div>"
        return h+"</div>"
    if v=='towers':
        ...
        for r in rs:
            ...
            h+=f"<div class=card><b>{esc(r['name'])}</b> {fixed}<br><small>{esc(r.get('location',''))}</small><br>احداثيات: {r.get('lat',0)},{r.get('lng',0)}<br>{delbtn}</div>"
        return h
    if v=='map':
        ...
        for t in fixed_towers: h+=f"<p><b>{esc(t['name'])}</b> - {esc(t['location'])} - {t['lat']},{t['lng']}</p>"
        h+="</div>"; return h
    if v=='logs':
        logs = qall("SELECT * FROM activity_log ORDER BY id DESC LIMIT 100")
        h="<div class=card><h3>سجل النشاطات</h3>"
        for log in logs: h+=f"<div style='border-bottom:1px solid #334155;padding:8px'><small>{esc(log['time'])} - {esc(log['phone'])}</small><br>{esc(log['action'])}</div>"
        return h+"</div>"
    if v=='settings':
        us=qall("SELECT phone,username,role FROM users ORDER BY phone");uh=""
        for u in us: ph=esc(u['phone']);un=esc(u.get('username') or u['phone']);uh+=f"<div class='card user-card'><div class=avatar>{esc(un[:1])}</div><div><b>{un}</b> ({esc(u.get('role'))})<br><small>{ph}</small></div></div>"
...
def layout(c,v='home'):
   ...
   .avatar{{width:40px;height:40px;border-radius:50%;background:#D4AF37;display:flex;align-items:center;justify-content:center;font-weight:bold}}
    ...
    <div class=top><span style='font-size:24px;cursor:pointer' onclick='toggleMenu()'>☰</span><span style='font-weight:800'>OMAIA ISP</span><div><button class=btn onclick="fetch('/toggle_lang').then(()=>location.reload())">EN/AR</button></div></div>
    ...
    window.loadPage=async function(v){{toggleMenu();document.getElementById('loader').style.display='flex';var r=await fetch('/api/page?v='+v);if(r.status==401){{location.href='/login';return}} document.getElementById('main').innerHTML=await r.text();document.getElementById('loader').style.display='none';bindAjax()}}
    function bindAjax(){{document.querySelectorAll('form[data-ajax]').forEach(f=>{{f.onsubmit=async e=>{{e.preventDefault();let res=await fetch(f.action,{{method:'POST',body:new FormData(f)}});if(res.status==401){{location.href='/login';return}} var currentPage=new URLSearchParams(window.location.search).get('v')||'home';loadPage(currentPage)}}}});document.querySelectorAll('a[data-del]').forEach(a=>{{a.onclick=async e=>{{e.preventDefault();if(confirm('تأكيد الحذف؟')){{let res=await fetch(a.href);if(res.status==401){{location.href='/login';return}} var currentPage=new URLSearchParams(window.location.search).get('v')||'home';loadPage(currentPage)}}}}}})}}
    ...
    function calcDist(){{
      ...
      if([lat1,lng1,lat2,lng2].some(isNaN)){out.innerText="ادخل الاحداثيات كاملة";return}
      out.innerText="المسافة: "+haversine(lat1,lng1,lat2,lng2).toFixed(2)+" كم";
    }}
    function pingDish(ip){{if(!ip){{alert('لا يوجد IP');return}} fetch('/api/ping?ip='+encodeURIComponent(ip)).then(r=>r.json()).then(j=>alert(j.out.slice(0,300)))}}
    bindAjax();
    </script></body></html>"""
    return p
...
@manager_required
def a4(i):
    name=qone("SELECT name FROM subs WHERE id=?",(i,));qexec("DELETE FROM subs WHERE id=?",(i,));add_log(f"حذف مشترك: {name['name'] if name else i}");return "ok"

@app.route('/add_dish',methods=['POST'])
@login_required
def c1():
    name=request.form.get('dish_name','');ip=request.form.get('ip','').strip();loc=request.form.get('location','')
    if not is_internal_ip(ip): return "IP غير داخلي",400
    qexec("INSERT INTO dish_ips(dish_name,ip,location) VALUES(?,?,?)",(name,ip,loc));add_log(f"اضافة طبق: {name} {ip}");return "ok"

@app.route('/del_dish/<int:i>')
@manager_required
def c2(i):
    name=qone("SELECT dish_name FROM dish_ips WHERE id=?",(i,));qexec("DELETE FROM dish_ips WHERE id=?",(i,));add_log(f"حذف طبق: {name['dish_name'] if name else i}");return "ok"
...
def d2(i):
    ...
    return "لا يمكن حذف نقطة ثابتة",403

@app.route('/add_user',methods=['POST'])
@manager_required
def addu():
    f=request.form;ph=f.get('phone','').strip()
    if qone("SELECT * FROM users WHERE phone=?",(ph,)): return "موجود مسبقا",400
    qexec("INSERT INTO users(phone,password,role,username,active) VALUES(?,?,?,?,1)",(ph,generate_password_hash(f.get('password','')),f.get('role','tech'),ph));add_log(f"اضافة مستخدم: {ph}");return "ok"

@app.route('/change_pass',methods=['POST'])
@login_required
def e2(): qexec("UPDATE users SET password=? WHERE phone=?",(generate_password_hash(request.form.get('newpass','')),session.get('phone')));add_log("تغيير كلمة السر");return "ok"
