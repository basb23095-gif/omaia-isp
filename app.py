...
def db():
    ...
    if USE_PG:
        now=time.time()
        if _pg and now-_last_check < 30: return _pg
        if _pg:
            try:
                cur=_pg.cursor();cur.execute("SELECT 1");cur.close()
                _last_check=now; return _pg
            except:
                try:_pg.close()
                except:pass
                _pg=None
        try:
            ...
            _pg.autocommit=True; _last_check=now; return _pg
        except: pass
    try:
...
def qexec(q,a=()):
    ...
    try:
        ...
        _cnt_cache.clear(); _stats_cache["ts"]=0
    except: cc(c)
...
def get_dish_table():
    now=time.time()
    if _dish_cache["name"] and now-_dish_cache["ts"] < 120: return _dish_cache["name"]
    if USE_PG:
        try:
            c=db(); cur=c.cursor(); cur.execute("SELECT to_regclass('public.ips')")
            r=cur.fetchone(); cur.close()
            tbl="ips" if r and r[0] else "dish_ips"
            _dish_cache["name"]=tbl; _dish_cache["ts"]=now; return tbl
...
def fast_count(tbl,ttl=20):
    ...
    _cnt_cache[tbl]=(c,now); return c

def get_home_stats():
    now=time.time()
    if _stats_cache["data"] and now-_stats_cache["ts"] < 20: return _stats_cache["data"]
    tbl=get_dish_table()
    try:
        ...
        if row: _stats_cache["data"]=row; _stats_cache["ts"]=now; return row
    except: pass
...
def is_valid_ip(ip):
    ...
    try: __import__('ipaddress').ip_address(ip); return True
    except: return len(ip)>=7 and '.' in ip

@app.route('/ping')
@app.route('/health')
def public_ping():
    try: c=db(); cur=c.cursor(); cur.execute("SELECT 1"); cur.close()
    except: pass
...
def api_ping():
    ...
    for port in [80,8291,8728]:
        s=None
        try:
            s=socket.socket(socket.AF_INET,socket.SOCK_STREAM); s.settimeout(0.45)
...
@login_required
def api_network():
    st=get_home_stats(); return jsonify(dishes=st.get('dishes',0),towers=st.get('towers',0),subs=st.get('subs',0))
...
def api_export(tbl):
    ...
    if tbl=='dishes':
        rows=qall(f"SELECT * FROM {dish_tbl} ORDER BY id DESC"); w.writerow(['ID','اسم','IP','موقع'])
        for r in rows: w.writerow([r['id'],r.get('dish_name',''),r.get('ip',''),r.get('location','')])
    elif tbl=='logs':
        rows=qall("SELECT * FROM logs ORDER BY id DESC LIMIT 2000"); w.writerow(['ID','مستخدم','عمل','تفاصيل','وقت'])
        for r in rows: w.writerow([r['id'],r.get('user_phone',''),r.get('action',''),r.get('detail',''),r.get('time','')])
    else:
        rows=qall(f"SELECT * FROM {tbl} ORDER BY id DESC LIMIT 500"); w.writerow(['ID','data'])
        for r in rows: w.writerow([r.get('id','')])
...
<div class=support>🛠 الدعم الفني<br><a href='https://wa.me/905345851045' target=_blank>واتساب +90 534 485 10 45</a> | <a href='https://instagram.com/af_20_1999' target=_blank>af_20_1999</a></div></div>
<script>
...
if(su) u.value=su;
if(localStorage.getItem('omaia_pass')) p.value=localStorage.getItem('omaia_pass');
document.getElementById('loginForm').addEventListener('submit',async e=>{
 ...
 try{
  ...
  let j=await r.json();
  if(j.ok){ localStorage.setItem('omaia_user',u.value); localStorage.setItem('omaia_pass',p.value); location.replace('/dash?v=home'); }
...
def s():
    ...
    try:
        for r in qall(f"SELECT * FROM {dish_tbl} WHERE ip LIKE? OR dish_name LIKE? OR location LIKE? ORDER BY id DESC LIMIT 20",(like,like,like)): results.append({"title":r.get('dish_name') or r.get('ip'),"sub":r.get('ip',''),"page":"dishes"})
...
def ad():
    ...
    if not ip or not is_valid_ip(ip): return "IP غير صالح",400
    ex=qone(f"SELECT * FROM {dish_tbl} WHERE ip=?",(ip,))
    if ex: qexec(f"UPDATE {dish_tbl} SET dish_name=?,location=? WHERE ip=?",(name,loc,ip))
    else: qexec(f"INSERT INTO {dish_tbl}(ip,location,dish_name) VALUES(?,?,?)",(ip,loc,name))
    add_log("اضافة/تعديل صحن",f"{name} {ip}"); return "ok"
...
def ed(i):
    ...
    add_log("تعديل صحن",str(i)); return "ok"

@app.route('/del_dish/<int:i>')
@login_required
def dd(i):
    if not is_manager(): return "ممنوع",403
    qexec(f"DELETE FROM {get_dish_table()} WHERE id=?",(i,)); add_log("حذف صحن",str(i)); return "ok"
...
def at():
    ...
    add_log("اضافة برج",request.form.get('name','')); return "ok"

@app.route('/del_tower/<int:i>')
@login_required
def dt(i):
    if not is_manager(): return "ممنوع",403
    qexec("DELETE FROM towers WHERE id=?",(i,)); add_log("حذف برج",str(i)); return "ok"
...
def et(i):
    ...
    add_log("تعديل برج",str(i)); return "ok"

@app.route('/add_sub',methods=['POST'])
@login_required
def asub():
    qexec("INSERT INTO subs(name,phone,note) VALUES(?,?,?)",(request.form.get('name',''),request.form.get('phone',''),request.form.get('note','')))
    add_log("اضافة مشترك",request.form.get('name','')); return "ok"

@app.route('/del_sub/<int:i>')
@login_required
def dsub(i):
    if not is_manager(): return "ممنوع",403
    qexec("DELETE FROM subs WHERE id=?",(i,)); add_log("حذف مشترك",str(i)); return "ok"
...
def esub(i):
    ...
    add_log("تعديل مشترك",str(i)); return "ok"
...
def al():
    ...
    add_log("اضافة حساب",request.form.get('name','')); return "ok"
...
def au():
    ...
    if not ph or qone("SELECT * FROM users WHERE phone=?",(ph,)): return "موجود",400
...
def eu():
    ...
    add_log("تعديل يوزر",new_ph); return "ok"
...
def du(ph):
    ...
    qexec("DELETE FROM users WHERE phone=?",(ph,)); add_log("حذف يوزر",ph); return "ok"
...
def page_content(v):
    ...
    def L(ar,en): return ar if (session.get('lang','ar')=='ar') else en
    if v=='home':
        st=get_home_stats()
        ...
        <div class=card><h4>📜 آخر النشاطات</h4>{log_html or 'لا يوجد'}<button class=btn-gold onclick="loadPage('logs')" style='width:100%;margin-top:8px'>عرض السجل الكامل</button></div></div>'''
    if v=='ping':
        return f'''<div style='max-width:800px;margin:0 auto'><div class=card><h3>📶 بنج نار - {dish_tbl}</h3>
        <div style='display:flex;gap:8px'><input id=pingIp placeholder='192.168.1.1' style='flex:1;padding:12px;background:#0f1424;border:1px solid #ffffff20;color:#fff;border-radius:10px'><button class=btn-gold onclick="doSinglePing()">Ping</button></div>
    ...
    if v=='dishes':
        ...
        for r in rs:
            ...
            rows+=f'<div class="card" id="dish-{rid}" data-name="{dn}" data-ip="{ip}" data-loc="{loc}" style="display:flex;justify-content:space-between"><div><b>{dn}</b><br><a href="http://{ip}" target="_blank" rel="noopener" style="color:#ffbe4d;font-family:monospace;text-decoration:none;background:#000;padding:4px 8px;border-radius:8px">🌐 {ip}</a><br><small style="color:#888">{loc}</small></div><div style="display:flex;gap:6px"><button class=btn-gold onclick="window.editDish({rid})" style="padding:7px 9px">✏</button><button class=btn-del onclick="askDel(\'/del_dish/{rid}\')">🗑</button></div></div>'
        return f'''<div style='max-width:900px;margin:0 auto'><div class=card><h3>📡 الصحون - {len(rs)}</h3><form data-ajax method=post action=/add_dish style='display:flex;gap:6px;flex-wrap:wrap'><input name=dish_name placeholder='اسم' required style='flex:1'><input name=ip placeholder='IP' required style='flex:1'><input name=location placeholder='موقع' style='flex:1'><button class=btn-gold>➕ حفظ فوري</button></form><input id=searchBox placeholder='🔍 بحث فوري...' oninput="searchDishes(this.value)" style='margin-top:10px;width:100%;padding:12px;border-radius:12px;background:#0f1424;border:1px solid #ffffff18'></div><div id=dl>{rows}</div></div><script>
        ...
        window.saveDish=function(id){{let nn=document.getElementById('ed_n').value; let ii=document.getElementById('ed_ip').value; let ll=document.getElementById('ed_l').value; fetch('/edit_dish/'+id,{{method:'POST',body:new URLSearchParams({{dish_name:nn,ip:ii,location:ll}})}}).then(()=>{{closeEditModal(); loadPage('dishes',true);}});}}
    ...
    if v=='towers':
        ...
        return f'''<div style='max-width:700px;margin:0 auto'><div class=card><h3>🗼 الأبراج</h3><form data-ajax method=post action=/add_tower style='display:flex;gap:6px;flex-wrap:wrap'><input name=name placeholder='اسم' required style='flex:1'><input name=area placeholder='منطقة' style='flex:1'><input name=lat placeholder='lat' style='width:80px'><input name=lng placeholder='lng' style='width:80px'><button class=btn-gold>➕ حفظ فوري</button></form></div>{rows}</div><script>
    ...
    if v=='subs':
        ...
        window.saveSub=function(id){{let n=document.getElementById('es_n').value; let p=document.getElementById('es_p').value; fetch('/edit_sub/'+id,{{method:'POST',body:new URLSearchParams({{name:n,phone:p,note:''}})}}).then(()=>{{closeEditModal(); loadPage('subs',true);}});}}
        </script>'''
    if v=='ledger':
        ...
        window.saveLed=function(id){{let n=document.getElementById('el_n').value; let a=document.getElementById('el_a').value; fetch('/edit_ledger/'+id,{{method:'POST',body:new URLSearchParams({{name:n,amount:a,note:'',currency:'USD'}})}}).then(()=>{{closeEditModal(); loadPage('ledger',true);}});}}
        </script>'''
    if v=='logs':
        ...
        return f"<div style='max-width:900px;margin:0 auto'><div class=card style='display:flex;justify-content:space-between'><h3>📜 السجل الكامل - {len(rs)}</h3><a href='/api/export/logs' class=btn-gold style='text-decoration:none'>📗 Excel</a></div>{rows or '<div class=card>لا يوجد سجل بعد - سيظهر هنا كل العمليات</div>'}</div>"
    if v=='network':
    ...
    if v=='search':
        q=request.args.get('q','').strip()
        if not q: return "<div class=card>اكتب كلمة بحث</div>"
        like="%"+q+"%"; results=[]; dish_tbl=get_dish_table()
        for r in qall(f"SELECT * FROM {dish_tbl} WHERE ip LIKE? OR dish_name LIKE? LIMIT 20",(like,like)): results.append(f"<div class=card><b>{esc(r.get('dish_name') or '')}</b> - <a href='http://{esc(r.get('ip') or '')}' target='_blank' style='color:#ffbe4d'>{esc(r.get('ip') or '')}</a></div>")
        for r in qall("SELECT * FROM towers WHERE name LIKE? LIMIT 20",(like,)): results.append(f"<div class=card>🗼 {esc(r['name'])} - {esc(r.get('area') or '')}</div>")
        return f"<div style='max-width:800px;margin:0 auto'><div class=card><h3>🔍 نتائج البحث عن: {esc(q)} - {len(results)}</h3></div>{''.join(results) or '<div class=card>لا يوجد</div>'}</div>"
...
.top-center{{font-weight:900;letter-spacing:0.5px}}
.top-right{{display:flex;gap:8px;align-items:center}}
.sidebar{{position:fixed;right:0;top:0;width:280px;height:100%;background:linear-gradient(180deg,#0f172a 0%,#0a1220 100%);z-index:1002;padding-top:70px;transform:translateX(110%);transition:transform 0.45s cubic-bezier(0.4,0,0.2,1), box-shadow 0.45s;overflow-y:auto;box-shadow:-8px 0 30px #0000}}
.sidebar.active{{transform:none;box-shadow:-12px 0 50px #0008}}
.sidebar a{{display:flex;gap:10px;padding:12px 14px;margin:6px 10px;color:#cbd5e1;text-decoration:none;border-radius:12px;background:#ffffff06;transition:all 0.3s cubic-bezier(0.4,0,0.2,1);transform:translateX(0)}}
.sidebar a:hover{{background:#ffffff12;transform:translateX(-6px)}}
.sidebar a.active{{background:linear-gradient(90deg,#ffbe4d,#ffb020);color:#111;font-weight:800;transform:translateX(-4px)}}
#overlay{{position:fixed;inset:0;background:#0009;backdrop-filter:blur(3px);z-index:1001;display:none;opacity:0;transition:opacity 0.4s ease}}#overlay.show{{display:block;opacity:1}}
.main{{margin-top:70px;padding:12px;transition:opacity 0.25s ease}}
.card{{background:#1e2433;padding:14px;border-radius:14px;margin-bottom:10px;border:1px solid #ffffff12;transition:transform 0.3s cubic-bezier(0.4,0,0.2,1), box-shadow 0.3s, background 0.3s}}
.card:hover{{transform:translateY(-2px);box-shadow:0 8px 24px #0004}}
input,select{{padding:11px;margin:5px 0;border-radius:10px;border:1px solid #ffffff12;width:100%;background:#ffffff07;color:#fff;transition:border 0.2s}}
input:focus{{border-color:#ffbe4d;outline:none}}
.btn-gold{{background:linear-gradient(90deg,#ffbe4d,#ffb020);color:#111;padding:8px 14px;border:0;border-radius:10px;font-weight:800;cursor:pointer;transition:all 0.25s cubic-bezier(0.4,0,0.2,1)}}
.btn-gold:hover{{transform:translateY(-1px) scale(1.02)}}
.btn-gold:active{{transform:scale(0.96)}}
.btn-del{{background:#ef4444;color:#fff;padding:7px 11px;border:0;border-radius:10px;transition:all 0.2s}}
#delModal,#editModal{{position:fixed;inset:0;background:#000a;backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;opacity:0;pointer-events:none;transition:opacity 0.35s ease, transform 0.35s;z-index:2000}}#delModal.show,#editModal.show{{opacity:1;pointer-events:auto}}
#delBox,#editBox{{background:#1e2433;padding:20px;border-radius:16px;width:92%;max-width:400px;transform:scale(0.92) translateY(20px);transition:transform 0.4s cubic-bezier(0.4,0,0.2,1)}}#delModal.show #delBox,#editModal.show #editBox{{transform:scale(1) translateY(0)}}
...
<div class=top>
<div class=top-right><span onclick="toggleSb()" style='font-size:24px;cursor:pointer;padding:6px 8px;border-radius:10px;background:#ffffff0a;transition:all 0.2s'>☰</span><div style='display:flex;gap:4px'><input id=topsearch placeholder='🔍 بحث...' style='background:#1f2937;border:1px solid #ffffff15;color:#fff;padding:8px 12px;border-radius:10px;width:42px;transition:all 0.35s cubic-bezier(0.4,0,0.2,1)' onfocus="this.style.width='160px'" onkeydown="if(event.key==='Enter'){{window.doTopSearch(this.value)}}"><button onclick="window.doTopSearch(document.getElementById('topsearch').value)" style='background:#ffbe4d;border:0;border-radius:10px;padding:8px 10px;cursor:pointer'>🔍</button></div></div>
<div class=top-center>OMAIA <span style='color:#ffbe4d'>ISP</span></div>
<div class=top-left><div id=notifBell onclick="toggleNotif()" style='position:relative;cursor:pointer;font-size:20px;padding:6px'>🔔<span id=notifCount style='display:none;position:absolute;top:-4px;right:-4px;background:#ef4444;color:#fff;font-size:10px;width:16px;height:16px;border-radius:50%;align-items:center;justify-content:center'>0</span></div><button onclick="window.toggleTheme()" title='ليل/نهار' style='background:#ffffff12;border:1px solid #ffffff15;color:#fff;padding:6px 10px;border-radius:10px;cursor:pointer;transition:all 0.2s'>🌓</button></div>
</div>
...
function toggleSb(f){{let sb=document.getElementById('sb'),ov=document.getElementById('overlay'); let o=f!==undefined?f:!sb.classList.contains('active'); sb.classList.toggle('active',o); ov.classList.toggle('show',o); ov.style.display=o?'block':'none'; if(o){{setTimeout(()=>ov.style.opacity='1',10);}} else {{ov.style.opacity='0'; setTimeout(()=>ov.style.display='none',400);}}}}
async function loadPage(v,force=false){{cur=v; toggleSb(false); document.querySelectorAll('.sidebar a').forEach(a=>a.classList.remove('active')); let n=document.getElementById('nav-'+v); if(n) n.classList.add('active'); let mn=document.getElementById('mn'); mn.style.opacity='0.6'; if(!force && pageCache[v]){{mn.innerHTML=pageCache[v]; bind(); execScripts(); mn.style.opacity='1'; return;}} try{{let r=await fetch('/api/page?v='+v); let h=await r.text(); pageCache[v]=h; mn.innerHTML=h; bind(); execScripts();
