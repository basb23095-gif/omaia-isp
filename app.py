...
def role_required(r='manager'):
    def dec(f):
        ...
        def w(*a,**kw):
            ...
            if not u:
                return redirect('/login')
            if r == 'manager' and u.get('role')!= 'manager':
                return "ممنوع: للمدير فقط",403
            return f(*a,**kw)
        return w
    return dec
...
def add_log(action):
    try:
        qexec("INSERT INTO logs(user,action) VALUES(?,?)",(session.get('phone','?'),action))
    except: pass
...
def api_ping():
    ...
    try:
        ...
        if ok:
            return jsonify(ok=True,out='متصل')
    except: pass
    for port in [80,443,8080,8291,22,8728]:
        try:
            ...
            if s.connect_ex((ip,port)) == 0:
                s.close()
                return jsonify(ok=True,out='متصل منفذ '+str(port))
            s.close()
        except: pass
...
def api_export(tbl):
    ...
    return Response(output.getvalue(),mimetype='text/csv',headers={'Content-Disposition':'attachment;filename='+tbl+'.csv'})
...
.card{background:#1e2433cc;border:1px solid #ffffff18;padding:25px;border-radius:20px;width:92%;max-width:360px}
...
<div class=card><form id=f><input name=userin placeholder='رقم / يوزر' required><input name=password type=password placeholder='كلمة السر' required><button class=btn id=b>دخول</button><div id=m style='color:#ef4444;text-align:center'></div></form>
...
<script>
document.getElementById('f').addEventListener('submit',async e=>{e.preventDefault();document.getElementById('loader').classList.add('show');document.getElementById('b').textContent='جاري التحميل...';let r=await fetch('/api/login_public',{method:'POST',body:new FormData(e.target)});let j=await r.json();if(j.ok){location.href='/dash?v=home';}else{document.getElementById('m').textContent='خطأ';document.getElementById('loader').classList.remove('show');document.getElementById('b').textContent='دخول';}});
</script></body></html>"""
...
@login_required
def at():
    qexec("INSERT INTO towers(name,area,lat,lng) VALUES(?,?,?,?)",(request.form.get('name',''),request.form.get('area',''),35.1318,36.7578))
    return "ok"
...
def eu():
    ...
    new = request.form.get('phone','').strip()
    pw = request.form.get('password','').strip()
    role = request.form.get('role','tech')
    if pw:
        qexec("UPDATE users SET phone=?,username=?,role=?,password=? WHERE phone=?",(new,new,role,generate_password_hash(pw),old))
    else:
        qexec("UPDATE users SET phone=?,username=?,role=? WHERE phone=?",(new,new,role,old))
    return "ok"
...
<div class='card stat' onclick="loadPage('net')"><div style='font-size:32px'>📶</div><h3>الشبكة</h3><h2>حية</h2></div>
</div>
<div class='card'><h4>📊 تصدير</h4><div style='display:flex;gap:8px;overflow-x:auto'>
...
</div><script>fetch('/api/net_status').then(r=>r.json()).then(d=>{let h='';d.forEach(x=>{h+='<div style="display:flex;justify-content:space-between;padding:6px;border-bottom:1px solid #ffffff10"><span>'+x.name+' '+x.ip+'</span><span>'+(x.ok?'🟢':'🔴')+'</span></div>'});document.getElementById('netLive').innerHTML=h||'لا يوجد';});</script>"""
    if v == 'dishes':
...
<input id=searchBox placeholder='🔍 بحث' oninput="doSearch(this.value)" style='margin-top:8px'></div><div id=dl></div></div>
<script>
async function doSearch(q){let r=await fetch('/api/search?q='+encodeURIComponent(q));let d=await r.json();let h='';d.forEach(x=>{h+='<div class="card" style="display:flex;justify-content:space-between;align-items:center"><div><b>'+x.dish_name+'</b><br><a href="http://'+x.ip+'" target="_blank" style="color:#ffbe4d;font-family:monospace">'+x.ip+'</a><br><small>'+(x.location||'')+'</small></div><div style="display:flex;gap:5px"><button class=btn-gold onclick="pingOne(\\''+x.ip+'\\',this)">📶</button><button class=btn-del onclick="askDel(\\'/del_dish/'+x.id+'\\')">🗑</button></div></div>'});document.getElementById('dl').innerHTML=h||'لا يوجد';}
async function pingOne(ip,btn){btn.textContent='...';let r=await fetch('/api/ping?ip='+encodeURIComponent(ip));let j=await r.json();btn.textContent=j.ok?'🟢':'🔴';}
window.searchDishes=doSearch;doSearch('');
</script>"""
    if v == 'towers':
        rs = qall("SELECT * FROM towers ORDER BY id DESC")
        rows = ""
        for r in rs:
            rows += "<div class='card' style='display:flex;justify-content:space-between;align-items:center'><div><b>🗼 "+esc(r['name'])+"</b><br><small>"+esc(r['area'] or '')+"</small></div><button class=btn-del onclick=\"askDel('/del_tower/"+str(r['id'])+"')\">🗑</button></div>"
        return "<div style='max-width:700px;margin:0 auto'><div class=card><h3>🗼 الأبراج</h3><form data-ajax method=post action=/add_tower style='display:flex;gap:6px'><input name=name placeholder='اسم' required style='flex:1'><input name=area placeholder='منطقة' style='flex:1'><button class=btn-gold>➕</button></form></div>"+rows+"</div>"
    if v == 'subs':
        rs = qall("SELECT * FROM subs ORDER BY id DESC LIMIT 200")
        rows = ""
        for r in rs:
            rows += "<div class='card' style='display:flex;justify-content:space-between'><div><b>"+esc(r['name'])+"</b><br>"+esc(r['phone'] or '')+"</div><button class=btn-del onclick=\"askDel('/del_sub/"+str(r['id'])+"')\">🗑</button></div>"
        return "<div style='max-width:700px;margin:0 auto'><div class=card><h3>👥 المشتركين</h3><form data-ajax method=post action=/add_sub style='display:flex;gap:5px'><input name=name placeholder='الاسم' required style='flex:1'><input name=phone placeholder='رقم' style='flex:1'><button class=btn-gold>➕</button></form></div>"+rows+"</div>"
    if v == 'ledger':
        rs = qall("SELECT * FROM ledger ORDER BY id DESC LIMIT 200")
        rows = ""
        for r in rs:
            rows += "<div class='card' style='display:flex;justify-content:space-between'><div><b>"+esc(r['name'])+"</b> "+str(r['amount'])+"</div><button class=btn-del onclick=\"askDel('/del_ledger/"+str(r['id'])+"')\">🗑</button></div>"
...
function loadNet(){document.getElementById('nl').innerHTML='جاري الفحص...';fetch('/api/net_status').then(r=>r.json()).then(d=>{let h='';d.forEach(x=>{h+='<div style="display:flex;justify-content:space-between;padding:10px;border-bottom:1px solid #ffffff10"><b>'+x.name+'<br><small>'+x.ip+'</small></b><span>'+(x.ok?'🟢 متصل':'🔴 واقف')+'</span></div>'});document.getElementById('nl').innerHTML=h;});}
loadNet();setInterval(loadNet,30000);</script>"""
    if v == 'map':
        towers = qall("SELECT * FROM towers")
        tj = json.dumps([{"name":t['name'],"lat":float(t.get('lat') or 35.1318),"lng":float(t.get('lng') or 36.7578)} for t in towers],ensure_ascii=False)
        return "<div class=card style='padding:6px'><div id=map style='height:72vh;border-radius:14px'></div></div><script>setTimeout(()=>{let map=L.map('map').setView([35.1318,36.7578],13);let osm=L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19}).addTo(map);let sat=L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',{maxZoom:19});L.control.layers({'عادية':osm,'قمر صناعي':sat}).addTo(map);let ts="+tj+";ts.forEach(t=>{L.marker([t.lat,t.lng]).addTo(map).bindPopup(t.name)});setTimeout(()=>map.invalidateSize(),400);},150);</script>"
    if v == 'logs':
        rs = qall("SELECT * FROM logs ORDER BY id DESC LIMIT 100")
        rows = ""
        for r in rs:
            rows += "<div class='card'><small>"+esc(str(r.get('at') or ''))+"</small><br><b>"+esc(r.get('user') or '')+"</b> - "+esc(r.get('action') or '')+"</div>"
        return "<div style='max-width:700px;margin:0 auto'><div class=card><h3>📝 السجل</h3></div>"+(rows or "<div class=card>فارغ</div>")+"</div>"
    if v == 'support':
        return """<div class=card style='text-align:center;max-width:500px;margin:0 auto'><h2>الدعم</h2><a href='https://wa.me/905344851045' style='display:inline-block;background:#22c55e;color:#fff;padding:14px 24px;border-radius:12px;text-decoration:none;font-weight:800'>واتساب</a></div>"""
    if v == 'settings':
        us = qall("SELECT * FROM users ORDER BY phone DESC")
        uh = ""
        for u in us:
            uh += "<div class='card'><b>"+esc(u['username'] or '')+"</b> "+esc(u['phone'])+" <small>"+esc(u['role'])+"</small></div>"
        return "<div style='max-width:700px;margin:0 auto'><div class=card><h3>⚙ الإعدادات</h3><button onclick=\"toggleLang()\" class=btn-gold>🌐 اللغة</button> <button onclick=\"toggleTheme()\" class=btn-gold>ليل/نهار</button></div><div class=card><h4>كلمة السر</h4><form data-ajax method=post action=/change_pass style='display:flex;gap:6px'><input name=newpass type=password placeholder='جديدة' style='flex:1'><button class=btn-gold>حفظ</button></form></div><div class=card><h4>اضافة يوزر</h4><form data-ajax method=post action=/add_user style='display:flex;flex-direction:column;gap:8px'><input name=user_field placeholder='رقم / يوزر' required><input name=password type=password placeholder='كلمة سر' required><select name=role><option value=tech>فني</option><option value=manager>مدير</option></select><button class=btn-gold>اضافة</button></form></div>"+uh+"</div>"
...
def layout(c,v='home'):
    ...
    cb = '#1e2433' if is_dark else '#ffffff'
    tx = '#ffffff' if is_dark else '#0f172a'
    bd = '#ffffff18' if is_dark else '#e2e8f0'
...
.top{{position:fixed;top:0;left:0;right:0;height:64px;background:#0f1424f2;color:#fff;display:flex;align-items:center;justify-content:space-between;padding:0 12px;z-index:1003}}
...
.main{{margin-top:74px;padding:12px;min-height:90vh}}
.card{{background:{cb};padding:14px;border-radius:18px;margin-bottom:10px;border:1px solid {bd};box-shadow:0 8px 24px #0005}}
.card.stat{{text-align:center;cursor:pointer}}.card.stat h2{{font-size:44px;color:#ffbe4d;margin:5px 0;font-weight:900}}
input,select{{padding:11px;border-radius:10px;border:1px solid {bd};width:100%;background:#ffffff08;color:{tx}}}
.btn-gold{{background:linear-gradient(90deg,#ffbe4d,#ffb020);color:#111;padding:8px 14px;border:0;border-radius:12px;font-weight:800;cursor:pointer;white-space:nowrap}}
.btn-del{{background:#ef4444;color:#fff;padding:8px 12px;border:0;border-radius:10px;cursor:pointer}}
...
<a href="javascript:loadPage('dishes')">📡 الصحون</a>
<a href="javascript:loadPage('net')">📶 حالة الشبكة</a>
<a href="javascript:loadPage('towers')">🗼 الأبراج</a>
<a href="javascript:loadPage('subs')">👥 المشتركين</a>
<a href="javascript:loadPage('ledger')">📒 الحسابات</a>
<a href="javascript:loadPage('map')">🗺 الخريطة</a>
<a href="javascript:loadPage('logs')">📝 السجل</a>
<a href="javascript:loadPage('settings')">⚙ الإعدادات</a>
<a href=/logout>🚪 خروج</a></div>
<div class=top><div style='display:flex;gap:8px;align-items:center'><span onclick="toggleSb()" style='font-size:24px;cursor:pointer'>☰</span><input id=topsearch placeholder='🔍 بحث' oninput="if(window.searchDishes)window.searchDishes(this.value)" style='width:90px;background:#1f2937;border:1px solid #374151;color:#fff;padding:7px;border-radius:10px'></div>
<div style='font-weight:900'>OMAIA <span style='color:#ffbe4d'>ISP</span></div>
<div><button onclick="toggleTheme()" style='background:#1f2937;color:#fff;border:0;padding:8px;border-radius:10px'>🌓</button></div></div>
<div class=main id=mn>{c}</div>
<a href='https://wa.me/905344851045' target=_blank style='position:fixed;bottom:20px;left:20px;background:#22c55e;color:#fff;width:56px;height:56px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:28px;z-index:999;text-decoration:none'>💬</a>
<div id=delModal><div style='background:{cb};padding:24px;border-radius:18px;width:90%;max-width:400px;text-align:center'><h3>تأكيد الحذف؟</h3><div style='display:flex;gap:10px'><button onclick="closeDel()" style='flex:1;padding:12px;border-radius:12px'>تراجع</button><button id=delYes style='flex:1;padding:12px;background:#ef4444;color:#fff;border:0;border-radius:12px'>حذف</button></div></div></div>
...
function toggleLang(){let l=localStorage.getItem('omaia_lang')||'ar';l=l==='ar'?'en':'ar';localStorage.setItem('omaia_lang',l);alert('OK '+l);}
