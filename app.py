...
def init():
    ...
    # إصلاح الجداول القديمة - يضيف الأعمدة الناقصة بدون حذف البيانات
    print("[MIGRATION] Fixing old tables...")
...
def fix_db_route():
    ...
    ensure_column("dish_ips", "ip", "TEXT")
    ensure_column("ips", "ip", "TEXT")
    return jsonify(ok=True, msg="تم إصلاح towers - area موجود الآن ✅")
...
def page_content(v):
    ...
    if v=='home':
        ...
        <div class=card style='margin-top:14px'><div style='display:flex;justify-content:space-between;flex-wrap:wrap'><h4>📊 {L('التقارير','Reports')} - <small style='color:#22c55e'>☁ : {dish_tbl}</small></h4><div style='display:flex;gap:8px'><a href='/api/export/dishes' class=btn-gold style='text-decoration:none;padding:8px 12px;background:linear-gradient(90deg,#22c55e,#16a34a);color:#fff'>📗 Excel</a><a href='/api/export/logs' class=btn-gold style='text-decoration:none;padding:8px 12px;background:linear-gradient(90deg,#8b5cf6,#7c3aed);color:#fff'>📜 Excel</a></div></div></div>
    ...
    if v=='ping':
        ...
        <h3 style='margin:0'>📶 {L('ping','Separate Ping')} 🔥 FIXED</h3>
    ...
    if v=='map':
        ...
        <div id=map style='height:60vh;min-height:380px;max-height:65vh;border-radius:16px;background:#0f172a;z-index:1;border:2px solid #ffffff0f'></div>
        <div style='margin-top:6px;font-size:11px;color:#6b7280;display:flex;gap:12px;flex-wrap:wrap'><span>{L('💡 اضغط على الخريطة','💡 Click map')}</span><span id=coordsLabel style='color:#ffbe4d'>📍 -</span></div>
        </div><script>
        let _towers={tj_json};
...
<div style='font-weight:900;font-size:16px'>OMAIA <span style='color:#ffbe4d'>ISP</span> <span style='color:#22c55e;font-size:10px'>☁ </span></div>
<div style='display:flex;gap:8px;align-items:center'>
