...
def dash():
 ...
 if v=='home':
  ...
  return F(title(T("home"),"🏠")+f"""<div class=home-search><input placeholder='🔍 بحث...' oninput=fS(this.value) style='text-align:left'></div><div class=g4><div class=c style=text-align:center><div style=font-size:28px>👥</div><h2 style=color:{M}>{ns}</h2>مشتركين</div><div class=c style=text-align:center><div style=font-size:28px>📡</div><h2 style=color:{M}>{nd}</h2>صحون</div><div class=c style=text-align:center><div style=font-size:28px>📒</div><h2 style=color:{M}>{nl}</h2>قيود</div><div class=c style=text-align:center><div style=font-size:28px>👤</div><h2 style=color:{M}>{nu}</h2>يوزرات</div></div><div class=c><div class=pt>📊 الأرباح</div><div style=display:flex;gap:10px><div style=flex:1;background:linear-gradient(135deg,#00D4FF,#0090c8);padding:14px;border-radius:12px;text-align:center;color:#021><b>💵 ${su}</b><br>دولار</div><div style=flex:1;background:linear-gradient(135deg,#ffd166,#ff9f1c);padding:14px;border-radius:12px;text-align:center;color:#021><b>💶 {sy}</b><br>سوري</div></div></div>""")
 if v=='subs':
 ...
 if v=='dishes':
  ...
  for x in rows:
   ...
   tr+=f"<tr><td dir=ltr><button onclick=cIP('{ip}') style='width:auto;padding:6px 10px'>📋 نسخ</button><br><a href='http://{ip}' target=_blank class=ip-small style=color:{M};text-decoration:none>🌐 {ip} ↗</a></td><td>📛 {d.get('name','')}</td><td>📶 {d.get('network','')}</td><td>🗼 {d.get('tower','')}</td><td><a href=/del_dish/{d['id']}>🗑️</a></td></tr>"
