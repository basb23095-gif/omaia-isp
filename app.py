...
<div style='display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px'><b>📦 الصحون {len(rs)} - كل برج معزول بكرت + كرت مخصص (Enter يحفظ/يبحث)</b><div style='display:flex;gap:6px'><button onclick="checkAllDishes()" class=btn-gold style='padding:6px 10px;background:#22c55e;color:#fff;font-size:11px'>فحص الكل</button><button onclick="createCustomCard()" class=btn-gold style='padding:6px 10px;background:#8b5cf6;color:#fff;font-size:11px'>+ كرت مخصص</button><a href='/api/export/dishes' class=btn-gold style='text-decoration:none;padding:6px 10px;font-size:11px'>Excel</a></div></div>
<div id=customCardCreator style='display:none;margin-top:10px;padding:12px;background:rgba(139,92,246,0.12);border:1px dashed #8b5cf6;border-radius:12px'><div style='display:flex;gap:6px;flex-wrap:wrap'><input id=customCardName placeholder='اسم الكرت - مثال: أبراج حماة' style='flex:1' onkeydown="if(event.key==='Enter'){{saveCustomCard();}}"><select id=customCardSize style='width:130px'><option value=small>📱 صغير</option><option value=medium selected>💻 وسط</option><option value=large>🖥️ كبير</option><option value=full>📺 كامل</option></select><button onclick="saveCustomCard()" class=btn-gold style='background:#8b5cf6;color:#fff'>حفظ الكرت</button><button onclick="document.getElementById('customCardCreator').style.display='none'" style='background:#ffffff15;color:#fff;border:0;padding:6px 12px;border-radius:8px'>إلغاء</button></div><div style='margin-top:10px'><small style='color:#aaa'>اختار الأبراج للعزل داخل الكرت الجديد:</small><div id=towerCheckboxes style='display:flex;gap:6px;flex-wrap:wrap;margin-top:6px'></div></div></div>
<form id=formDish style='display:flex;gap:6px;margin-top:10px;flex-wrap:wrap'><input name=dish_name id=dish_name_input placeholder='اسم الصحن - Enter يحفظ' required style='flex:1' onkeydown="if(event.key==='Enter'){{this.form.requestSubmit();}}"><input name=ip id=dish_ip_input placeholder='192.168.1.1 - Enter يحفظ' required style='flex:1' onkeydown="if(event.key==='Enter'){{this.form.requestSubmit();}}"><input name=location id=dish_loc_input placeholder='البرج / موقع - Enter يحفظ' style='flex:1' onkeydown="if(event.key==='Enter'){{this.form.requestSubmit();}}"><button class=btn-gold type=submit id=btnAddDish>إضافة (Enter)</button></form>
<input id=searchBox placeholder='بحث... Enter يبحث' oninput="searchDishes(this.value)" onkeydown="if(event.key==='Enter'){{searchDishes(this.value);}}" style='margin-top:10px'>
</div>
<div id=customCardsContainer style='display:grid;gap:12px;margin-bottom:12px'></div>
<div id=towerGroups style='display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:12px'></div>
...
window.createCustomCard=function(){{ let el=document.getElementById('customCardCreator'); el.style.display=el.style.display==='none'?'block':'none'; let box=document.getElementById('towerCheckboxes'); if(!box) return; box.innerHTML=''; let towers=[...new Set([...document.querySelectorAll('#dl .dish-card')].map(c=>c.dataset.loc))].filter(Boolean); towers.forEach(t=>{{ box.innerHTML+='<label style="background:rgba(255,255,255,0.08);padding:6px 10px;border-radius:8px;font-size:12px;cursor:pointer;border:1px solid rgba(255,255,255,0.1)"><input type=checkbox value="'+t+'" checked style="margin-left:6px"> '+t+'</label>'; }}); if(!towers.length) box.innerHTML='<small style="color:#888">لا يوجد أبراج بعد - أضف صحن أولا</small>'; }};
window.saveCustomCard=function(){{ let name=document.getElementById('customCardName').value.trim()||'كرت مخصص'; let size=document.getElementById('customCardSize').value; let selected=[...document.querySelectorAll('#towerCheckboxes input:checked')].map(i=>i.value); if(!selected.length){{alert('اختر برج واحد على الأقل');return;}} let container=document.getElementById('customCardsContainer'); let sizeStyle=size==='small'?'max-width:380px':size==='medium'?'max-width:600px':size==='large'?'max-width:850px':'max-width:100%'; let card=document.createElement('div'); card.className='card'; card.style.cssText='border:2px solid #8b5cf6;'+sizeStyle+';background:linear-gradient(135deg,rgba(139,92,246,0.15),rgba(30,36,58,0.9));backdrop-filter:blur(14px);animation:fadeIn .4s ease;transform:translateZ(0)'; let inner=''; selected.forEach(t=>{{ let g=document.querySelector('.tower-group[data-tower="'+t+'"]'); if(g) inner+=g.outerHTML; }}); card.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:6px"><b style="color:#8b5cf6">📦 '+name+' ('+selected.length+' أبراج)</b><div style="display:flex;gap:4px"><button onclick="this.closest(\'.card\').style.maxWidth=\'380px\'" style="padding:3px 8px;font-size:10px;background:#ffffff15;color:#fff;border:0;border-radius:6px">صغير</button><button onclick="this.closest(\'.card\').style.maxWidth=\'600px\'" style="padding:3px 8px;font-size:10px;background:#ffffff15;color:#fff;border:0;border-radius:6px">وسط</button><button onclick="this.closest(\'.card\').style.maxWidth=\'100%\'" style="padding:3px 8px;font-size:10px;background:#8b5cf6;color:#fff;border:0;border-radius:6px">كبير</button><button onclick="this.closest(\'.card\').remove()" style="background:rgba(239,68,68,0.15);color:#ef4444;border:0;padding:4px 10px;border-radius:6px">✕ حذف الكرت</button></div></div><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:10px">'+inner+'</div>'; container.prepend(card); document.getElementById('customCardCreator').style.display='none'; document.getElementById('customCardName').value=''; }};
(function(){{ // تجميع كل برج معزول بكرت مع اختيار حجم
  let grouped={{}};
  document.querySelectorAll('#dl .dish-card').forEach(c=>{{ let tower=(c.dataset.loc||'بدون برج').trim()||'بدون برج'; if(!grouped[tower]) grouped[tower]=[]; grouped[tower].push(c.outerHTML); }});
  let tg=document.getElementById('towerGroups');
  if(!tg) return;
  tg.innerHTML='';
  Object.keys(grouped).sort().forEach(tower=>{{
    let count=grouped[tower].length;
    let div=document.createElement('div');
    div.className='tower-group card';
    div.dataset.tower=tower;
    div.style.cssText='background:linear-gradient(135deg,rgba(30,36,58,0.9),rgba(18,24,42,0.95));border:1px solid rgba(255,190,77,0.18);backdrop-filter:blur(12px);border-radius:16px;overflow:hidden;transform:translateZ(0);transition:all .3s cubic-bezier(.2,.8,.2,1)';
    div.innerHTML='<div style="background:linear-gradient(90deg,#ffbe4d,#ffb020);color:#111;padding:10px 14px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px"><div><b>📡 '+tower+'</b> <span style="background:#111;color:#ffbe4d;padding:2px 8px;border-radius:10px;font-size:11px">'+count+' IP</span></div><div style="display:flex;gap:4px;align-items:center"><button onclick="this.closest(\'.tower-group\').style.gridColumn=\'span 1\'" style="padding:3px 8px;font-size:10px;background:#111;color:#fff;border:0;border-radius:6px">صغير</button><button onclick="this.closest(\'.tower-group\').style.gridColumn=\'span 2\'" style="padding:3px 8px;font-size:10px;background:#111;color:#fff;border:0;border-radius:6px">وسط</button><button onclick="this.closest(\'.tower-group\').style.gridColumn=\'1 / -1\'" style="padding:3px 8px;font-size:10px;background:#8b5cf6;color:#fff;border:0;border-radius:6px">كبير</button><button onclick="pingTower(\''+tower+'\')" style="padding:4px 10px;font-size:10px;background:#22c55e;color:#fff;border:0;border-radius:6px">فحص</button></div></div><div class="tower-ips" style="padding:10px;display:grid;gap:8px;max-height:400px;overflow-y:auto">'+grouped[tower].join('')+'</div><div style="padding:8px;background:rgba(0,0,0,0.2);display:flex;gap:6px"><input placeholder="اسم صحن - Enter" style="flex:1;padding:6px;font-size:11px" onkeydown="if(event.key==='Enter'){{addQuickIp(this,\''+tower+'\');}}"><input placeholder="IP - Enter" style="flex:1;padding:6px;font-size:11px" onkeydown="if(event.key==='Enter'){{addQuickIp(this,\''+tower+'\');}}"><button onclick="addQuickIp(this,\''+tower+'\')" style="background:#ffbe4d;color:#111;border:0;padding:6px 12px;border-radius:6px;font-size:11px">+ (Enter)</button></div>';
    tg.appendChild(div);
  }});
  if(!Object.keys(grouped).length){{ tg.innerHTML='<div class="card" style="grid-column:1/-1;text-align:center;padding:30px;color:#888">لا يوجد أبراج بعد<br><small>أضف أول صحن مع اسم برج - كل برج رح ينعزل بكرت لحال مع IPs تبعو</small></div>'; }}
}})();
window.pingTower=async function(tower){{ let cards=document.querySelectorAll('.tower-group[data-tower="'+tower+'"] .dish-card'); for(let c of cards){{ let id=c.id.split('-')[1]; if(window.pingOneDish){{ await pingOneDish(id); await new Promise(r=>setTimeout(r,120)); }} }} }};
