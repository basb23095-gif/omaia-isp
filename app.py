...
"""
    ...
    if v=='dishes':
        ...
        for r in rs:
            ...
            # Glass Tech Card
            rows_html += f'''
<div class="card dish-card glass-tech" id="dish-{rid}" data-name="{dn}" data-ip="{ip}" data-loc="{loc}" style="background:linear-gradient(135deg,rgba(30,36,58,0.85) 0%,rgba(18,24,42,0.9) 100%);border:1px solid rgba(100,150,255,0.18);box-shadow:0 0 0 1px rgba(100,150,255,0.08),0 8px 32px rgba(0,0,0,0.4),0 0 20px rgba(59,130,246,0.12);backdrop-filter:blur(12px);border-radius:16px;padding:16px;display:flex;justify-content:space-between;align-items:center;transition:.2s">
<div style="flex:1;min-width:0">
<div style="display:flex;align-items:center;gap:8px"><span class="status-dot" id="dot-{rid}" style="width:10px;height:10px;border-radius:50%;background:#555;display:inline-block;box-shadow:0 0 6px #555;flex-shrink:0"></span><b class="dish-name" style="font-size:17px;font-weight:900;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{dn}</b></div>
<div style="margin:6px 0 0 18px;color:#94a3b8;font-size:12px;display:flex;align-items:center;gap:6px"><span>📡</span><span class="dish-loc">{loc or 'بدون موقع'}</span></div>
<div style="margin-top:8px"><a class="dish-ip" href="http://{ip}" target="_blank" style="display:inline-block;background:linear-gradient(90deg,#ffbe4d,#ffb020);color:#111;padding:5px 14px;border-radius:8px;font-family:monospace;font-size:13px;font-weight:800;text-decoration:none;box-shadow:0 2px 10px #ffbe4d44">{ip}</a></div>
</div>
<div style="display:flex;flex-direction:column;gap:6px;align-items:stretch;margin-right:8px">
<button class="btn-gold" onclick="pingOneDish({rid})" style="padding:7px 14px;background:linear-gradient(90deg,#22c55e,#16a34a);color:#fff;border-radius:8px;font-size:12px;font-weight:700;white-space:nowrap">Ping</button>
<div style="display:flex;gap:5px"><button class="btn-gold" onclick="editDish({rid})" style="padding:6px 10px;background:#1f2937;color:#fff;border:1px solid #ffffff15;border-radius:8px;font-size:11px;flex:1">تعديل</button><button class="btn-del" onclick="askDel('/del_dish/{rid}',{rid},'dish')" style="padding:6px 10px;border-radius:8px;font-size:11px;flex:1">حذف</button></div>
</div>
</div>'''
        return f"""
<div style='max-width:900px;margin:0 auto'>
<div class=card style='background:rgba(30,36,58,0.6);border:1px solid rgba(100,150,255,0.15);backdrop-filter:blur(10px)'>
<div style='display:flex;justify-content:space-between;align-items:center'><b>الصحون - {len(rs)} - Glass Tech</b><div style='display:flex;gap:6px'><button onclick="checkAllDishes()" class=btn-gold style='padding:6px 10px;background:#22c55e;color:#fff;font-size:11px'>فحص الكل</button><a href='/api/export/dishes' class=btn-gold style='text-decoration:none;padding:6px 10px;font-size:11px'>Excel</a></div></div>
...
<div style="flex:1"><b class="t-name" style="font-size:16px;font-weight:900;color:#fff">{name}</b><br><span class="t-area" style="color:#94a3b8;font-size:12px">📍 {area or 'بدون منطقة'}</span><br><span class="t-coord" style="color:#ffbe4d;font-family:monospace;font-size:11px;background:rgba(255,190,77,0.1);padding:2px 8px;border-radius:6px;margin-top:4px;display:inline-block">{lat},{lng}</span></div>
