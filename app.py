...
def do_ping(ip):
    try:
        ...
        out=subprocess.run(['ping',param,'1','-W','1',ip],capture_output=True,text=True,timeout=3)
        txt=(out.stdout+out.stderr)[:300]
        ok=out.returncode==0
        return ok, txt
...
def page_content(v):
    ...
    if v=='dishes':
        return f"""<div style='max-width:900px;margin:0 auto'><div class=card style='text-align:center'><h3>📡 {L('dishes')}</h3><form id=dishForm style='display:flex;gap:6px;flex-wrap:wrap;justify-content:center'><input name=dish_name required placeholder='اسم الصحن' style='max-width:150px'><input name=ip required placeholder='IP' style='max-width:150px'><input name=location placeholder='موقع' style='max-width:150px'><button class=btn-gold type=submit>{L('add')}</button></form><div style='margin-top:8px'><button class=btn-blue onclick='window.open("/export_excel")'>📊 Excel</button> <button class=btn-blue onclick='window.open("/export_pdf")'>📄 PDF</button></div></div><div id=dl style='display:grid;grid-template-columns:1fr 1fr;gap:10px'></div><div style='text-align:center;margin:10px'><button class=btn onclick='mD()'>المزيد</button></div><script>
let pg=0;
async function lD(){{let r=await fetch('/api/search?page='+pg);let d=await r.json();let el=document.getElementById('dl');let h='';d.forEach(x=>{{let eb="{'<button class=icon-btn style=background:#FF9800 onclick=eD('+chr(39)+'"+x.id+"'+chr(39)+')>✏️</button>' if ce else ''}";let db="{'<button class=icon-btn style=background:#F44336 onclick=delItem('+chr(39)+'/del_dish/'+chr(39)+'+x.id)>🗑️</button>' if cd else ''}";h+='<div class=card style="display:flex;justify-content:space-between;align-items:center"><div><b>'+x.dish_name+'</b><br><span class=ip-badge>'+x.ip+'</span><br><small>'+(x.location||'')+'</small></div><div style="display:flex;gap:6px;flex-direction:column">'+eb+db+'<button class=btn-blue onclick="pingDish(\\''+x.ip+'\\')">بينغ</button></div></div>'}});if(pg==0)el.innerHTML=h;else el.innerHTML+=h}}
function mD(){{pg++;lD()}}lD();
document.getElementById('dishForm').onsubmit=async e=>{{e.preventDefault();let fd=new FormData(e.target);let r=await fetch('/add_dish',{{method:'POST',body:fd}});if(r.ok){{toast('تم ✅');e.target.reset();pg=0;lD()}}else toast('خطأ')}};
async function eD(id){{let n=prompt('اسم جديد:');if(n==null)return;await fetch('/edit_dish/'+id,{{method:'POST',body:new URLSearchParams({{dish_name:n}})}});toast('تم');pg=0;lD()}}
</script></div>"""
    if v=='towers':
        rs=qall("SELECT * FROM towers ORDER BY id DESC LIMIT 50");cards=""
        for r in rs:
            eb=f"<button class=icon-btn style='background:#FF9800' onclick='eT({r['id']})'>✏️</button>" if ce else "";db=f"<button class=icon-btn style='background:#F44336' onclick='delItem(\"/del_tower/{r['id']}\")'>🗑️</button>" if cd else ""
        ...
        return f"<div style='max-width:900px;margin:0 auto'><div class=card style='text-align:center'><h3>🗼 {L('towers')}</h3><form data-ajax method=post action=/add_tower><input name=name required placeholder='اسم البرج'><input name=area placeholder='المنطقة'><button class=btn-gold>{L('add')}</button></form></div><div style='display:grid;grid-template-columns:1fr 1fr;gap:10px'>{cards}</div><script>async function eT(id){{let n=prompt('اسم جديد:');if(!n)return;await fetch('/edit_tower/'+id,{{method:'POST',body:new URLSearchParams({{name:n}})}});loadPage('towers',true)}}</script></div>"
    if v=='subs':
        rs=qall("SELECT * FROM subs ORDER BY id DESC LIMIT 50");rows=""
        for r in rs:
            eb=f"<button class=icon-btn style='background:#FF9800' onclick='eS({r['id']})'>✏️</button>" if ce else "";db=f"<button class=icon-btn style='background:#F44336' onclick='delItem(\"/del_sub/{r['id']}\")'>🗑️</button>" if cd else ""
        ...
        return f"<div style='max-width:600px;margin:0 auto'><div class=card style='text-align:center'><h3>👥 {L('subs')}</h3><form data-ajax method=post action=/add_sub><input name=name required placeholder='الاسم'><input name=phone placeholder='هاتف'><button class=btn-gold>{L('add')}</button></form></div>{rows}<script>async function eS(id){{let n=prompt('اسم:');if(!n)return;await fetch('/edit_sub/'+id,{{method:'POST',body:new URLSearchParams({{name:n}})}});loadPage('subs',true)}}</script></div>"
    if v=='ledger':
        rs=qall("SELECT * FROM ledger ORDER BY id DESC LIMIT 50");rows=""
        for r in rs:
            eb=f"<button class=icon-btn style='background:#FF9800' onclick='eL({r['id']})'>✏️</button>" if ce else "";db=f"<button class=icon-btn style='background:#F44336' onclick='delItem(\"/del_ledger/{r['id']}\")'>🗑️</button>" if cd else ""
    ...
    if v=='map':
        ...
        return "<div class=card><div style='margin-bottom:6px'><button class=btn-blue onclick=\"toast('استخدم مسطرة القياس')\">📏 قياس</button> <small>كليك لتثبيت نقطة - يمين لإضافة برج</small></div><div id=map style='height:70vh;border-radius:12px'></div><link rel='stylesheet' href='https://cdn.jsdelivr.net/gh/gokertanrisever/leaflet-ruler@master/src/leaflet-ruler.css'><script src='https://cdn.jsdelivr.net/gh/gokertanrisever/leaflet-ruler@master/src/leaflet-ruler.js'></script><script>var _t="+tj+";var map=L.map('map',{zoomControl:true}).setView([35.1318,36.7578],16);L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',{maxZoom:19,maxNativeZoom:19}).addTo(map);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,opacity:0.4}).addTo(map);L.control.ruler().addTo(map);setTimeout(()=>map.invalidateSize(),300);_t.forEach(t=>L.marker([t.lat,t.lng]).addTo(map).bindPopup(t.name));var tmp=null;map.on('click',e=>{if(tmp)map.removeLayer(tmp);tmp=L.marker(e.latlng,{draggable:true}).addTo(map).bindPopup(e.latlng.lat.toFixed(6)+','+e.latlng.lng.toFixed(6)).openPopup()});map.on('contextmenu',async e=>{let n=prompt('اسم البرج:');if(!n)return;await fetch('/add_tower',{method:'POST',body:new URLSearchParams({name:n,lat:e.latlng.lat,lng:e.latlng.lng})});toast('تم')});</script></div>"
    if v=='logs':
...
def layout(c,v='home'):
    ...
    tmpl="""<html dir=__DIR__><head><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'><link rel='stylesheet' href='https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'><style>*{box-sizing:border-box;font-family:sans-serif}body{margin:0;background:__BG__;color:#fff;transition:background.3s}body.light{background:#f5f5f5;color:#111}body.light.card{background:#fff;color:#111;border-color:#ddd}body.light input{background:#fff;color:#111;border-color:#ccc}.sidebar{position:fixed;right:-285px;top:0;width:275px;height:100%;background:#111827f5;z-index:1000;padding-top:70px;transition:right.35s cubic-bezier(.4,0,.2,1);backdrop-filter:blur(14px);box-shadow:-10px 0 40px #0006}.sidebar.active{right:0}.sidebar a{display:flex;gap:12px;padding:14px 18px;color:#fff;text-decoration:none;border-radius:14px;margin:5px 10px;transition:.2s}.sidebar a:hover{background:#ffffff1a;transform:translateX(-4px)}.overlay{position:fixed;inset:0;background:#0007;display:none;z-index:999;backdrop-filter:blur(2px)}.overlay.active{display:block}.top{position:fixed;top:0;left:0;right:0;height:62px;background:#111827ee;display:flex;align-items:center;justify-content:space-between;padding:0 12px;z-index:101;backdrop-filter:blur(12px)}.main{margin-top:62px;padding:10px;animation:fd.25s}@keyframes fd{from{opacity:0;transform:translateY(10px)}to{opacity:1}}.card{background:__CARD__;padding:14px;border-radius:18px;margin-bottom:10px;border:1px solid #ffffff12;box-shadow:0 6px 20px #0003}.card:active{transform:scale(.99)}input,select{width:100%;padding:11px;margin:5px 0;background:#0f1424;border:1px solid #ffffff20;color:#fff;border-radius:12px}.btn-gold{background:__GOLD__;color:#000;padding:11px 18px;border:0;border-radius:12px;font-weight:bold;cursor:pointer}.btn-blue{background:#2196F3;color:#fff;padding:9px 14px;border:0;border-radius:10px;cursor:pointer}.btn{background:#333;color:#fff;padding:9px 14px;border:0;border-radius:10px;cursor:pointer}.icon-btn{width:38px;height:38px;border:0;border-radius:12px;cursor:pointer;font-size:16px}.ip-badge{background:#000;color:__GOLD__;padding:4px 10px;border-radius:20px;font-family:monospace}#toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:#1f2937;padding:12px 22px;border-radius:30px;display:none;z-index:9999}.support-btn{display:inline-flex;gap:8px;color:#fff;padding:13px 26px;border-radius:30px;text-decoration:none;margin:6px;font-weight:bold}.top-left{display:flex;gap:8px}.top-btn{background:#ffffff15;border:0;color:#fff;padding:8px 12px;border-radius:20px;cursor:pointer}</style></head><body><div id=toast></div><div class=overlay id=ov onclick="tgM(false)"></div><div class=sidebar id=sb><a href="javascript:loadPage('home')">🏠 __HOME__</a><a href="javascript:loadPage('dishes')">📡 __DISHES__</a><a href="javascript:loadPage('towers')">🗼 __TOWERS__</a><a href="javascript:loadPage('subs')">👥 __SUBS__</a><a href="javascript:loadPage('ledger')">📒 __LEDGER__</a><a href="javascript:loadPage('map')">🗺 __MAP__</a><a href="javascript:loadPage('logs')">📜 __LOGS__</a><a href="javascript:loadPage('support')">💬 __SUPPORT__</a><a href="javascript:loadPage('settings')">⚙️ __SETTINGS__</a><a href=/logout>🚪 __LOGOUT__</a></div><div class=top><div style='font-size:24px;cursor:pointer' onclick="tgM()">☰</div><div>__LOGO__</div><div class=top-left><button class=top-btn onclick="tL()">🌐 __LANG__</button><button class=top-btn onclick="tT()">🌓</button></div></div><div class=main id=mn>__CONTENT__</div><script src='https://unpkg.com/leaflet@1.9.4/dist/leaflet.js'></script><script>
...
async function loadPage(v,f){cur=v;tgM(false);let mn=document.getElementById('mn');if(cache[v]&&!f){mn.innerHTML=cache[v];ex();return}mn.style.opacity='0.4';let r=await fetch('/api/page?v='+v);let h=await r.text();cache[v]=h;mn.innerHTML=h;mn.style.opacity='1';ex()}
function ex(){document.getElementById('mn').querySelectorAll('script').forEach(s=>{try{eval(s.textContent)}catch(e){console.log(e)}});bd()}
function bd(){document.querySelectorAll('form[data-ajax]').forEach(f=>{if(f._b)return;f._b=1;f.onsubmit=async e=>{e.preventDefault();await fetch(f.action,{method:'POST',body:new FormData(f)});for(let k in cache)delete cache[k];toast('تم ✅');loadPage(cur,true)}})}
...
window.pingDish=async function(ip){toast('بينغ '+ip+'...');let r=await fetch('/api/ping?ip='+encodeURIComponent(ip));let j=await r.json();toast(j.out.slice(0,150))};
window.tT=async()=>{await fetch('/toggle_theme');document.body.classList.toggle('light');toast('تم')};
...
def login():
    ...
    return """<html dir=rtl><head><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'><style>body{margin:0;min-height:100vh;background:linear-gradient(180deg,#0a0e2a,#1a1446);display:flex;flex-direction:column;align-items:center;justify-content:center;font-family:sans-serif;color:#fff}.card{background:#1e2433ee;padding:28px;border-radius:22px;width:330px;border:1px solid #ffffff15}input{width:100%;padding:13px;margin:8px 0;background:#0f1424;border:1px solid #ffffff20;color:#fff;border-radius:12px;box-sizing:border-box}.btn{width:100%;padding:14px;border:0;border-radius:14px;background:linear-gradient(90deg,#3b9dff,#8b5cf6);color:#fff;font-weight:bold;font-size:17px;margin-top:10px;cursor:pointer}.support{margin-top:18px;text-align:center}.support a{display:inline-flex;gap:8px;background:#25D366;color:#fff;padding:10px 20px;border-radius:30px;text-decoration:none;font-weight:bold}</style></head><body><div style='font-size:52px'>🌐</div><div style='font-size:26px;font-weight:800;background:linear-gradient(90deg,#5aa9ff,#c084fc);-webkit-background-clip:text;-webkit-text-fill-color:transparent'>شركة أمية للإنترنت</div><div class=card><h3 style='text-align:center;color:#5aa9ff'>🔐 تسجيل الدخول</h3><form method=post><input name=userin placeholder='📱 الهاتف / المستخدم' required><input name=password type=password placeholder='🔑 كلمة السر' required><label style='font-size:13px;color:#aaa'><input type=checkbox name=remember style='width:auto'> حفظ كلمة السر</label><button class=btn>✨ دخول</button></form></div><div class=support><div style='margin-bottom:8px;color:#aaa'>الدعم الفني</div><a href='https://wa.me/905344851045' target=_blank>💬 واتساب</a></div></body></html>"""
@app.route('/logout')
...
def ad():
    ...
    if qone("SELECT * FROM dish_ips WHERE ip=?",(ip,)): return "IP موجود",400
    qexec("INSERT INTO dish_ips(ip,location,dish_name) VALUES(?,?,?)",(ip,request.form.get('location',''),nm));add_log("اضافة صحن "+ip);return "ok"
...
@app.route('/backup')
@manager_required
def bk(): return jsonify(ok=True,use_pg=USE_PG,count=qone("SELECT COUNT(*) AS c FROM dish_ips"))
