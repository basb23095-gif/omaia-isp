...
def get_view_html(v,c,role):
 ...
 if v=='settings':
  ...
  return f"<style>.u-grid{{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:8px}}.ucard{{background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);border-radius:14px;padding:10px;text-align:center;font-size:10px}}.badge{{font-size:9px!important;padding:2px 8px;border-radius:20px;color:#fff}}.badge.green{{background:#22c55e}}.badge.gray{{background:#6b7280}}.ubtns{{display:flex;gap:4px;justify-content:center;margin-top:7px;flex-wrap:wrap}}.mini-btn{{font-size:10px!important;padding:5px 9px!important;border-radius:8px;text-decoration:none;color:#fff;font-weight:700;display:inline-block}}.mini-btn.blue{{background:#3b82f6}}.mini-btn.orange{{background:#f59e0b}}.mini-btn.red{{background:#ef4444}}.mini-btn.gray{{background:#6b7280}}.modal{{position:fixed;top:0;right:0;left:0;bottom:0;background:rgba(0,0,0,.6);z-index:2000;align-items:center;justify-content:center}}.modal-box{{background:#1a2332;padding:14px;border-radius:14px;width:90%;max-width:300px}}</style><div class='card glass eye' style='text-align:center'><h3>⚙️ الإعدادات</h3><p style='opacity:.7'>تصميم م. عبدو عباس<br><a href='https://wa.me/{SUPPORT}' target='_blank' style='color:#4da3ff;text-decoration:underline' dir=ltr>{SUPPORT_DISPLAY} 💬</a></p></div><div class='card glass eye'><h3>⚙️ إضافة مستخدم</h3><form method=post action=/add_user><input name=phone placeholder='رقم الهاتف' required dir=ltr><div class=row2><input name=password placeholder='كلمة السر' required><select name=role><option value='tech'>فني</option><option value='admin'>مدير</option><option value='distributor'>موزع</option></select></div><button class='btn-soft'>+ إضافة</button></form></div><div class='u-grid'>{cards}</div>"
 if v=='support': return f"<div class='card glass eye' style='text-align:center'><h3>🛠️ الدعم</h3><a href='https://wa.me/{SUPPORT}' target='_blank' style='color:#4da3ff;text-decoration:underline'><h2 dir=ltr>{SUPPORT_DISPLAY} 💬</h2></a><button class='btn-soft' onclick=\"window.open('https://wa.me/{SUPPORT}','_blank')\">💬 واتساب</button><p style='margin-top:10px;opacity:.6'>تصميم م. عبدو عباس</p></div>"
 return ""
...
.wa{{position:fixed;bottom:14px;left:14px;width:52px;height:52px;border-radius:50%;background:#25D366;display:flex!important;align-items:center;justify-content:center;font-size:26px;text-decoration:none;z-index:9999;box-shadow:0 4px 12px rgba(0,0,0,.4);cursor:pointer}}
...
<div style='text-align:center;padding:20px 10px 60px;font-size:10px;opacity:.9'>تصميم م. عبدو عباس<br><a href='https://wa.me/{SUPPORT}' target='_blank' style='color:#4da3ff;text-decoration:underline' dir=ltr>{SUPPORT_DISPLAY} 💬</a></div>
<a class="wa" href="https://wa.me/{SUPPORT}" target="_blank" onclick="window.open('https://wa.me/{SUPPORT}','_blank')">💬</a>
