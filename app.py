from flask import Flask,request,redirect,render_template_string,session,send_from_directory,make_response
...
CSS="body{font-family:Arial;margin:0;background:__BG__;color:__TEXT__}body.lb{background:linear-gradient(rgba(5,11,24,.6),rgba(5,11,24,.7)),url('/bg.jpg') center/cover fixed,__BG__;min-height:100vh;display:flex;align-items:center;justify-content:center}.t{position:fixed;top:0;left:0;right:0;height:50px;background:__SIDEBAR__;display:flex;align-items:center;justify-content:space-between;padding:0 10px;z-index:9}.m{padding:60px 10px;max-width:1050px;margin:auto}.c{background:__CARD__;border:1px solid __MAIN__;border-radius:10px;padding:12px;margin:8px 0}button{background:__MAIN__;border:0;padding:9px;width:100%;border-radius:8px;font-weight:bold;cursor:pointer}input,select{width:100%;padding:8px;margin:4px 0;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#fff;box-sizing:border-box}table{width:100%;border-collapse:collapse}td,th{padding:7px;border-bottom:1px solid #334155;text-align:center;font-size:14px}th{color:__MAIN__}.g4{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.g2{display:grid;grid-template-columns:1fr 1fr;gap:8px}@media(max-width:700px){.g4{grid-template-columns:1fr 1fr}.g2{grid-template-columns:1fr}}.n a{color:#fff;margin:0 5px;text-decoration:none;font-size:14px}"
LAY="<!DOCTYPE html><html dir=rtl><head><meta charset=UTF-8><meta name=viewport content='width=device-width,initial-scale=1'><title>OMAIA</title><style>"+CSS+"</style></head><body class=__BC__><div class=t><b style=color:__MAIN__>OMAIA</b><div class=n><a href=/dash>🏠</a><a href=/dash?view=subs>👥</a><a href=/dash?view=dishes>📡</a><a href=/dash?view=ledger>📒</a><a href=/dash?view=settings>⚙️</a><a href=/logout>🚪</a></div></div><div class=m>{{c|safe}}</div></body></html>"
...
def login():
 if request.method=='POST':
  ...
  if d and d['password']==request.form.get('password') and d['active']:
   session['p']=d['phone'];session['r']=d['role'];return redirect('/dash')
  return R("<div class=c style='width:320px;text-align:center'><p style=color:red>خطأ</p><a href=/>رجوع</a></div>","lb")
 return R("<div class=c style='width:320px;text-align:center'><h2 style=color:#00D4FF>OMAIA ISP</h2><form method=post><input name=phone placeholder='رقم هاتف / اسم مستخدم' required autocomplete=username><input name=password type=password placeholder='باسورد' required autocomplete=current-password><button>دخول</button></form></div>","lb")
...
def dash():
 ...
 if v=='home':
  ...
  return F(f"<div class=g4><div class=c style=text-align:center><h2 style=color:{M}>{ns}</h2>مشتركين</div><div class=c style=text-align:center><h2 style=color:{M}>{nd}</h2>صحون</div><div class=c style=text-align:center><h2 style=color:{M}>{nl}</h2>دفتر</div><div class=c style=text-align:center><h2 style=color:{M}>{nu}</h2>يوزرات</div></div><div class=g2><div class=c><b>آخر المشتركين</b><table>{ts}</table></div><div class=c><b>آخر الصحون</b><table>{td}</table></div></div><div class=c>مرحبا {session['p']}</div>")
 if v=='subs':
 ...
 if v=='dishes':
  ...
  return F(f"<div class=c><form method=post action=/add_dish><input name=location placeholder='شبكة' required><input name=region placeholder='المنطقة'><input name=ip placeholder='IP' dir=ltr required><button>+ صحن</button></form></div><div class=c><table><tr><th>شبكة</th><th>منطقة</th><th>IP</th><th></th></tr>{tr}</table></div>")
 if v=='ledger':
...
def e1(i):
 ...
 return R(f"<div class=c><form method=post><input name=name value='{r['name']}' required><input name=phone value='{r['phone']}'><button>حفظ</button></form></div>")
...
def e4(p):
 ...
 return R(f"<div class=c><form method=post><input name=ident value='{u['username']}' required><input name=password value='{u['password']}' required><select name=role><option value=tech>فني</option><option value=super>super</option></select><button>حفظ</button></form></div>")
