...
def init():
    ...
    for s in ss: qexec(s)
    if not qone("SELECT * FROM settings WHERE k='allow_edit'"):
        qexec("INSERT INTO settings(k,v) VALUES('allow_edit','1')")
    if not qone("SELECT * FROM settings WHERE k='allow_del'"):
        qexec("INSERT INTO settings(k,v) VALUES('allow_del','1')")
...
def get_set(k):
    r=qone("SELECT * FROM settings WHERE k=?",(k,))
    return r['v'] if r else '1'

def add_log(a):
    ph=session.get('phone','system')
    u=qone("SELECT username FROM users WHERE phone=?",(ph,))
    un=u['username'] if u else ph
...
def page_content(v):
    ...
    if v=='home':
        ...
        return f"<div style='max-width:700px;margin:0 auto;text-align:center'><div style='font-size:28px'>{logo_html()}</div><div class=card><input id=hs placeholder='{L('search')}' oninput='hSrch(this.value)' style='max-width:400px;margin:0 auto'><div id=hr></div></div><div style='display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:12px'><div class=card onclick=\"loadPage('subs')\" style='cursor:pointer'><h3>{L('subs')}</h3><h2>{ns}</h2></div><div class=card onclick=\"loadPage('dishes')\" style='cursor:pointer'><h3>{L('dishes')}</h3><h2>{nd}</h2></div><div class=card onclick=\"loadPage('towers')\" style='cursor:pointer'><h3>{L('towers')}</h3><h2>{nt}</h2></div><div class=card onclick=\"loadPage('map')\" style='cursor:pointer'><h3>{L('map')}</h3><h2>map</h2></div></div></div><script>async function hSrch(v){{if(v.length<2){{document.getElementById('hr').innerHTML='';return}}let r=await fetch('/api/search?q='+encodeURIComponent(v));let d=await r.json();document.getElementById('hr').innerHTML=d.map(x=>'<div>'+x.dish_name+' '+x.ip+'</div>').join('')}}</script>"
    if v=='dishes':
        return f"""<div style='max-width:900px;margin:0 auto'><div class=card style='text-align:center'><h3>{L('dishes')}</h3><form id=df style='display:flex;gap:6px;flex-wrap:wrap;justify-content:center'><input name=dish_name required placeholder='اسم' style='max-width:150px'><input name=ip required placeholder='IP' style='max-width:150px'><input name=location placeholder='موقع' style='max-width:150px'><button class=btn-gold type=submit>{L('add')}</button></form></div><div id=dl style='display:flex;flex-direction:column;gap:6px;align-items:center'></div><div style='text-align:center'><button class=btn onclick='mD()'>المزيد</button></div><script>
let pg=0;
async function lD(){{let r=await fetch('/api/search?page='+pg);let d=await r.json();let h='';d.forEach(x=>{{
let eb="";let db="";
if("{str(ce)}"=="True") eb='<button class="icon-btn" style="background:#FF9800" onclick="eD('+x.id+')">E</button>';
if("{str(cd)}"=="True") db='<button class="icon-btn" style="background:#F44336" onclick="dD('+x.id+')">D</button>';
h+='<div class=card style="padding:6px 10px;display:flex;justify-content:space-between;align-items:center;width:340px;max-width:100%"><div style="font-size:13px"><b>'+x.dish_name+'</b> <span class=ip-badge>'+x.ip+'</span></div><div style="display:flex;gap:4px">'+eb+db+'<button class=btn-blue onclick="pN(\\''+x.ip+'\\')">Ping</button></div></div>'}});
let el=document.getElementById('dl');if(pg==0)el.innerHTML=h;else el.innerHTML+=h;}}
function mD(){{pg++;lD()}}lD();
async function pN(ip){{let r=await fetch('/api/ping?ip='+ip);let j=await r.json();toast(j.out)}}
async function dD(id){{if(!confirm('حذف؟'))return;await fetch('/del_dish/'+id);pg=0;lD()}}
async function eD(id){{let n=prompt('اسم جديد:');if(!n)return;await fetch('/edit_dish/'+id,{{method:'POST',body:new URLSearchParams({{dish_name:n}})}});pg=0;lD()}}
document.getElementById('df').onsubmit=async e=>{{e.preventDefault();let r=await fetch('/add_dish',{{method:'POST',body:new FormData(e.target)}});if(r.ok){{pg=0;lD();e.target.reset()}}else toast('خطأ')}};
</script></div>"""
    if v=='ping':
    ...
    if v=='map':
        ...
        tj=json.dumps([{"name":t['name'],"lat":float(t.get('lat') or 35.1318),"lng":float(t.get('lng') or 36.7578)} for t in tw],ensure_ascii=False)
...
def layout(c,v='home'):
    th=cur_theme()
    bg=COLORS.get('bg_dark','#0a1938')
    card=COLORS.get('card_dark','#1e2433')
    gold=COLORS.get('gold','#ffbe4d')
    lg=logo_html()
    lang=cur_lang()
    t=T[lang]
    dirv='rtl' if lang=='ar' else 'ltr'
    return f"""<html dir={dirv}><head><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'>
<link rel='stylesheet' href='https://unpkg.com/leaflet@1.9.4/dist/leaflet.css'>
<style>
*{{box-sizing:border-box;font-family:sans-serif}}
body{{margin:0;background:{bg};color:#fff}}
body.light{{background:#f5f5f5;color:#111}}
body.light.card{{background:#fff;color:#111}}
body.light.sidebar{{background:rgba(255,255,255,0.95)}}
body.light.sidebar a{{color:#111}}
.sidebar{{position:fixed;right:-285px;top:0;width:275px;height:100%;background:rgba(17,24,39,0.88);z-index:1000;padding-top:70px;transition:right.3s;backdrop-filter:blur(18px);border-left:1px solid #ffffff18}}
.sidebar.active{{right:0}}
.sidebar a{{display:block;padding:13px 18px;color:#fff;text-decoration:none;border-radius:10px;margin:4px 10px}}
.sidebar a:hover{{background:#ffffff1f}}
.overlay{{position:fixed;inset:0;background:#0006;display:none;z-index:999}}
.overlay.active{{display:block}}
.top{{position:fixed;top:0;left:0;right:0;height:60px;background:rgba(17,24,39,0.9);display:flex;align-items:center;justify-content:space-between;padding:0 12px;z-index:101;backdrop-filter:blur(16px)}}
.main{{margin-top:62px;padding:12px}}
.card{{background:{card};padding:14px;border-radius:14px;margin-bottom:10px;border:1px solid #ffffff12}}
input,select{{width:100%;padding:11px;margin:5px 0;background:#0f1424;border:1px solid #ffffff20;color:#fff;border-radius:10px}}
body.light input,body.light select{{background:#fff;color:#111;border-color:#ddd}}
.btn-gold{{background:{gold};color:#000;padding:10px 16px;border:0;border-radius:10px;font-weight:bold;cursor:pointer}}
.btn-blue{{background:#2196F3;color:#fff;padding:8px 12px;border:0;border-radius:8px;cursor:pointer}}
.btn{{background:#333;color:#fff;padding:8px 14px;border:0;border-radius:8px;cursor:pointer}}
.icon-btn{{width:32px;height:32px;border:0;border-radius:8px;cursor:pointer;color:#fff}}
.ip-badge{{background:#000;color:{gold};padding:3px 8px;border-radius:12px;font-family:monospace;font-size:12px}}
#toast{{position:fixed;bottom:20px;left:50%;transform:translateX(-50%);background:#222;color:#fff;padding:10px 20px;border-radius:20px;display:none;z-index:9999}}
.top-btn{{background:#ffffff15;border:0;color:#fff;padding:8px 12px;border-radius:16px;cursor:pointer}}
</style></head><body>
<div id=toast></div>
...
<a href="javascript:loadPage('ping')">{t['ping']}</a>
<a href="javascript:loadPage('map')">{t['map']}</a>
<a href="javascript:loadPage('settings')">{t['settings']}</a>
<a href=/logout>{t['logout']}</a>
</div>
<div class=top><div onclick="tgM()" style='font-size:24px;cursor:pointer'>☰</div><div>{lg}</div><div><button class=top-btn onclick="tL()">{lang.upper()}</button> <button class=top-btn onclick="tT()">🌓</button></div></div>
...
async function loadPage(v){{tgM(false);let r=await fetch('/api/page?v='+v);document.getElementById('mn').innerHTML=await r.text();document.getElementById('mn').querySelectorAll('script').forEach(s=>{{try{{eval(s.textContent)}}catch(e){{}}}})}}
async function tT(){{await fetch('/toggle_theme');location.reload()}}
async function tL(){{await fetch('/toggle_lang');location.reload()}}
...
def s():
    ...
    off=pg*20
    if q:
        return jsonify(qall(f"SELECT * FROM dish_ips WHERE ip LIKE? OR dish_name LIKE? ORDER BY id DESC LIMIT 20 OFFSET {off}",("%"+q+"%","%"+q+"%")))
    return jsonify(qall(f"SELECT * FROM dish_ips ORDER BY id DESC LIMIT 20 OFFSET {off}"))

@app.route('/toggle_theme')
@login_required
def tt():
    session['theme']='light' if cur_theme()=='dark' else 'dark'
    return "ok"

@app.route('/toggle_lang')
@login_required
def tl():
    session['lang']='en' if cur_lang()=='ar' else 'ar'
    return "ok"
...
def ad():
    ...
    nm=request.form.get('dish_name','').strip()
    if not ip or not nm: return "ناقص",400
    if qone("SELECT * FROM dish_ips WHERE ip=?",(ip,)): return "موجود",400
    qexec("INSERT INTO dish_ips(ip,location,dish_name) VALUES(?,?,?)",(ip,request.form.get('location',''),nm))
...
def ed(i):
    ...
    qexec("UPDATE dish_ips SET dish_name=? WHERE id=?",(request.form.get('dish_name',''),i))
    return "ok"
...
@login_required
def at():
    qexec("INSERT INTO towers(name,area,lat,lng) VALUES(?,?,?,?)",(request.form.get('name',''),request.form.get('area',''),float(request.form.get('lat') or 35.1318),float(request.form.get('lng') or 36.7578)))
    return "ok"
...
@login_required
def al():
    qexec("INSERT INTO ledger(name,note,amount,currency,dt) VALUES(?,?,?,?,?)",(request.form.get('name',''),request.form.get('note',''),fnum(request.form.get('amount')),request.form.get('currency','USD'),datetime.datetime.now().isoformat()))
    return "ok"
...
def au():
    ...
    qexec("INSERT INTO users(phone,password,role,username,active) VALUES(?,?,?,?,1)",(ph,generate_password_hash(request.form.get('password','')),request.form.get('role','tech'),ph))
    return "ok"

@app.route('/edit_user/<ph>',methods=['POST'])
@manager_required
def eu(ph):
    qexec("UPDATE users SET role=? WHERE phone=?",(request.form.get('role','tech'),ph))
    return "ok"
