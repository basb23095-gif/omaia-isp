...
def init():
    ...
    ss = [
        ...
        "CREATE TABLE IF NOT EXISTS dish_ips(id INTEGER PRIMARY KEY AUTOINCREMENT,ip TEXT,location TEXT,area TEXT,lat REAL,lng REAL)",
...
def view_html(v,c):
    ...
    if v == 'home':
        ...
        if USE_PG:
            n1 = dict(ex(c,"SELECT COUNT(*) as c FROM subs").fetchone())['c']
            n2 = dict(ex(c,"SELECT COUNT(*) as c FROM dish_ips").fetchone())['c']
        else:
            n1 = list(ex(c,"SELECT COUNT(*) as c FROM subs").fetchone())[0]
            n2 = list(ex(c,"SELECT COUNT(*) as c FROM dish_ips").fetchone())[0]
        c1 = col.get('icon_ip','#3b82f6')
        c2 = col.get('icon_active','#8b5cf6')
        html = "<div class='card' style='text-align:center'><h2>امية ISP</h2><p>"+today+"</p></div>"
        html += "<div class='igrid'><div class='icard' style='background:"+c1+"'>صحون<br>"+str(n2)+"</div>"
        html += "<div class='icard' style='background:"+c2+"'>مشتركين<br>"+str(n1)+"</div></div>"
        return html
    ...
    if v == 'subs':
        ...
        for r in rs:
            d = dict(r)
            b = "<a class='abtn abtn-del' href='/del_sub/"+str(d['id'])+"'>حذف</a>" if edit else "<small>عرض فقط</small>"
            rows += "<tr><td>"+str(d.get('name',''))+"</td><td>"+str(d.get('phone',''))+"</td><td>"+b+"</td></tr>"
        return "<div class='card'><h3>المشتركين</h3><form method=post action=/add_sub><div class=row2><input name=name placeholder='الاسم' required><input name=phone placeholder='هاتف' required></div><button class='btn-soft'>اضافة</button></form></div><div class='card'><table><tr><th>اسم</th><th>هاتف</th><th>اجراء</th></tr>"+rows+"</table></div>"
    ...
    if v == 'towers':
        ...
        for r in rs:
            d = dict(r)
            b = "<a class='abtn abtn-del' href='/del_tower/"+str(d['id'])+"'>حذف</a>" if edit else ""
            rows += "<tr><td>"+str(d.get('name',''))+"</td><td>"+b+"</td></tr>"
        return "<div class='card'><h3>الابراج</h3><form method=post action=/add_tower><input name=name placeholder='اسم البرج' required><div class=row2><input name=lat placeholder='Lat' type=number step=any required><input name=lng placeholder='Lng' type=number step=any required></div><button class='btn-soft'>اضافة برج</button></form></div><div class='card'><table>"+rows+"</table></div>"

    return "<div class=card>اهلا</div>"
...
def base_html(content):
    ...
    bg = get_bg_css()
    logo = get_logo_html()
    return "<!DOCTYPE html><html dir='rtl'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>OMAIA</title><style>"+css+" *{box-sizing:border-box} body{margin:0;font-family:'Segoe UI';"+bg+";color:"+col['text']+"}.card{padding:12px;border-radius:16px;margin:8px 0;background:"+col['card_bg']+";border:1px solid rgba(255,255,255,.1)}.igrid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.icard{padding:16px;border-radius:12px;color:#fff;text-align:center}.abtn{padding:6px 12px;border-radius:10px;color:#fff;text-decoration:none;display:inline-block}.abtn-edit{background:#06b6d4}.abtn-del{background:#ef4444}.abtn-toggle{background:#f59e0b}.btn-soft{width:100%;padding:11px;border:none;border-radius:12px;background:linear-gradient(135deg,"+col['main']+","+col['accent']+");color:#fff;font-weight:800} input{width:100%;padding:10px;margin:5px 0;border-radius:10px;border:1px solid rgba(255,255,255,.2);background:rgba(255,255,255,.06);color:"+col['text']+"}.row2{display:grid;grid-template-columns:1fr 1fr;gap:8px} table{width:100%;border-collapse:collapse} th,td{padding:8px;text-align:center;font-size:12px}.mn{padding:70px 10px;max-width:1200px;margin:auto}.top{position:fixed;top:0;right:0;left:0;height:56px;display:flex;align-items:center;justify-content:space-between;padding:0 12px;z-index:1000;background:rgba(0,0,0,.3);backdrop-filter:blur(10px)} </style></head><body><div class='top'><b>"+logo+" OMAIA ISP</b><div><a href='/dash?v=home' style='color:"+col['link']+"'>الرئيسية</a> <a href='/dash?v=map' style='color:"+col['link']+"'>الخريطة</a> <a href='/dash?v=subs' style='color:"+col['link']+"'>مشتركين</a> <a href='/dash?v=dishes' style='color:"+col['link']+"'>صحون</a> <a href='/dash?v=towers' style='color:"+col['link']+"'>ابراج</a> <a href='/logout' style='color:"+col['link']+"'>خروج</a></div></div><div class='mn'>"+content+"</div></body></html>"
