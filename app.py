...
def init_pg_pool():
    ...
    with _pg_pool_lock:
        ...
        try:
            ...
            print("[POOL] Postgres pool created 1-20")
...
def get_conn():
    """يرجع connection من الـ pool - سريع نار"""
    if USE_PG:
        if _pg_pool:
            try:
...
def add_perf_headers(resp):
    # كاش للمتصفح - صفحات ديناميكية لا تتخزن لكن الأصول تتخزن
    ...
    if USE_PG:
        try:
            if _pg_pool:
                ...
                conn = psycopg2.connect(DATABASE_URL, sslmode='require', connect_timeout=2)
                conn.close()
                pg_ok = True
        except Exception as e:
            err = str(e)[:300]
...
#loader{position:fixed;inset:0;background:#0a0e2acc;backdrop-filter:blur(6px);z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;opacity:0;pointer-events:none;transition:opacity .2s}
...
<div id=loader><div class=spinner></div><div style='margin-top:12px;color:#ffbe4d;font-weight:800'>⏳ جاري تسجيل الدخول...</div></div>
...
document.getElementById('loginForm').addEventListener('submit',async e=>{
 ...
 btn.textContent='⏳ ثواني...'; btn.disabled=true; msg.textContent='';
 ...
 try{
  ...
  if(j.ok){
    ...
    msg.textContent=j.msg||'خطأ'; btn.textContent='✨ دخول فوري'; btn.disabled=false; loader.classList.remove('show');
  }
 }catch(err){
  msg.textContent='خطأ شبكة - جرب مرة ثانية'; btn.textContent='✨ دخول فوري'; btn.disabled=false; loader.classList.remove('show');
 }
});
</script></body></html>"""
...
def page_content(v):
    ...
    if v=='ping':
        ...
        <p style='color:#9ca3af;font-size:12px;margin:6px 0'>✅ TCP أولا (شغال على Render) - {dish_tbl} ☁ Pool: 20</p>
...
<a href="javascript:loadPage('ping')" id=nav-ping onmouseenter="prefetchPage('ping')" style='background:linear-gradient(90deg,#22c55e18,#16a34a18);border:1px solid #22c55e33'>📶 {L('بنج منفصل','Separate Ping')} <span style='background:#22c55e;color:#fff;padding:2px 6px;border-radius:8px;font-size:10px;margin-right:auto'>FAST</span></a>
<a href="javascript:loadPage('network')" id=nav-network onmouseenter="prefetchPage('network')">📊 {L('حالة الشبكة','Network')} <span style='background:#0ea5e9;color:#fff;padding:2px 6px;border-radius:8px;font-size:10px;margin-right:auto'>LIVE</span></a>
<a href="javascript:loadPage('dishes')" id=nav-dishes onmouseenter="prefetchPage('dishes')">📡 {L('الصحون','Dishes')} <span style='background:#22c55e;color:#fff;padding:2px 6px;border-radius:6px;font-size:9px'>☁</span></a>
...
<a href="javascript:loadPage('logs')" id=nav-logs onmouseenter="prefetchPage('logs')">📜 {L('السجل','Logs')}</a>
<a href="javascript:loadPage('map')" id=nav-map onmouseenter="prefetchPage('map')">🗺 {L('الخريطة الحية','Live Map')} <span style='background:#f59e0b;color:#fff;padding:2px 6px;border-radius:8px;font-size:10px;margin-right:auto'>HD</span></a>
...
async function loadPage(v,force=false,push=true){{
  ...
  // عرض فوري من الكاش بدون انتظار - سلس نار
  if(!force && pageCache[v]){{
    mn.innerHTML=pageCache[v];
    bind(); execScripts();
    // تحديث بالخلفية بصمت
    fetch('/api/page?v='+v+'&lang='+lang,{{cache:'no-store'}}).then(r=>r.text()).then(h=>{{ if(h && h.length>50){{ pageCache[v]=h; saveCache(); }} }}).catch(()=>{{}});
    return;
  }}

  // لو ما في كاش - اعرض هيكل سريع
  mn.innerHTML='<div class=card style="height:120px" ><div class="skeleton" style="height:20px;border-radius:8px;width:40%;margin-bottom:12px"></div><div class="skeleton" style="height:14px;border-radius:6px;margin-bottom:8px"></div><div class="skeleton" style="height:14px;border-radius:6px;width:80%"></div></div><div class=card style="height:80px"><div class="skeleton" style="height:100%;border-radius:12px"></div></div>';
  ...
  }}catch(e){{ mn.innerHTML='<div class=card>❌ '+e+'<br><button class=btn-gold onclick="loadPage(\\''+v+'\\',true)">↻ اعادة محاولة</button></div>'; }}
}}
  ...
  document.querySelectorAll('form[data-ajax]').forEach(f=>{{
    ...
    f.onsubmit=async e=>{{
      ...
      try{{
        ...
        if(r.ok){{delete pageCache[cur]; await loadPage(cur,true,false);}}
...
document.getElementById('delYes').onclick=async()=>{{if(window._delUrl){{let r=await fetch(window._delUrl); if(!r.ok){{let t=await r.text(); alert(t); closeDel(); return;}} delete pageCache[cur]; closeDel(); loadPage(cur,true,false);}}}};
...
window.logoutFast=async function(){{ try{{await fetch('/api/logout',{{method:'POST'}});}}catch(e){{}} try{{localStorage.removeItem('omaia_cache_v3'); localStorage.removeItem('omaia_cache_v4');}}catch(e){{}} location.replace('/login'); }};
...
bind(); execScripts();
// تحميل مسبق ذكي - يحمل كل الصفحات بالخلفية بعد ثانية
setTimeout(()=>{{
  let pages=['home','dishes','towers','subs','ledger','ping','network','map'];
  let i=0;
  function next(){{ if(i>=pages.length) return; let p=pages[i++]; if(!pageCache[p]){{ fetch('/api/page?v='+p+'&lang='+lang,{{cache:'no-store'}}).then(r=>r.text()).then(h=>{{pageCache[p]=h; saveCache(); setTimeout(next,300);}}).catch(()=>setTimeout(next,300)); }} else {{ setTimeout(next,100); }} }}
  if('requestIdleCallback' in window) requestIdleCallback(next,{{timeout:2000}}); else next();
}},1200);
